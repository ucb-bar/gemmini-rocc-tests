...
void full_right_chol(void *arg){
	int p;
	p = *((int *)arg);
	elem_t * LR_pt = (elem_t *) LR_block;
	elem_t * L = LR_pt + block_dim*(MAT_DIM*p+p);
...
int main() {
 ...
 int cpu_id;
 cpu_id = sched_getcpu();
 printf("Starting block right CPU chol - cpu_id: %d \n", cpu_id);
 cpu_set_t cpuset;
 ...
 cpu_start = read_cycles();
  ...
  for(int k = 0; k < num_block; k++){
		for(int i = 0; i < num_proc; i++){
			...
			if(i==0) pthread_create(&thread[i], &attr, full_right_chol, (void*) &k);
			else pthread_create(&thread[i], &attr, print_message, NULL); 
			//pthread_exit(&attr);
		}
		for(int i = 0; i < num_proc; i++)
		...
		for(int i = 0; i < num_proc; i++){
			...
			matmul_args[i].temp = (elem_t *)temp;
 			...
 			CPU_SET(i, &cpuset); //add each cpu to cpu set
			pthread_attr_setaffinity_np(&attr, sizeof(cpu_set_t), &cpuset);
			pthread_create(&thread[i], &attr, lower_triangle_inverse, &matmul_args);
			//pthread_exit(&attr);
		}
		for(int i = 0; i < num_proc; i++)
			pthread_join(thread[i], NULL);
    ...
    cpu_end = read_cycles();
    printf("Cycles taken(cpu_id: %d): %u\n", cpu_id, cpu_end-cpu_start);
