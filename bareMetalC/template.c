...
int main() {
  ...
  size_t In_sp_addr = 0;
  size_t Out_sp_addr = DIM_ROWS;
  size_t Identity_sp_addr = 2*DIM_ROWS;

  printf("Move \"In\" matrix from main memory into Gemmini's scratchpad\n");
  gemmini_config_ld(DIM * sizeof(elem_t));
  // gemmini_mvin(In, In_sp_addr);
  gemmini_extended_mvin(In, In_sp_addr, DIM, DIM_ROWS);
