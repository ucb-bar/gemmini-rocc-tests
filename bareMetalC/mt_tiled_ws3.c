...
void thread_entry(int cid, int nc)
{
  ...
  elem_t* B = (elem_t*) in_B + b_unit*DIM*(cid);
  elem_t* C = (elem_t*) Out + b_unit*DIM*(cid);// + MAT_DIM_J*DIM*(cid/2);
  acc_t * D = (acc_t*) bias + b_unit*DIM*(cid);// + MAT_DIM_J*DIM*(cid/2);
  ...
  */
  ...
  for(int j = 0; j < nc; j++){
		//printf("thread: %d, loop: %d \n", cid, j);
	 if(j == cid)
	  tiled_matmul_auto_distance(MAT_DIM_I, MAT_DIM_J, MAT_DIM_K, DIM, b_unit*DIM*nc, 1, 4,
				A, B, NO_BIAS ? NULL : D, C,
			   A_STRIDE, B_STRIDE, MAT_DIM_J, MAT_DIM_J,
            MVIN_SCALE_IDENTITY, MVIN_SCALE_IDENTITY, MVIN_SCALE_IDENTITY,
            NO_ACTIVATION, ACC_SCALE_IDENTITY, 0, REPEATING_BIAS,
