...
void thread_entry(int cid, int nc){
 ...
 for(int i = 0; i < num_block; i++)
  ...
  for(int k = 0; k < num_block; k++){
		if(cid == 0) full_right_chol(block_size, LR_pt+block_size*(MAT_DIM*k+k));
		barrier(nc);
		elem_t temp[block_size][block_size];
		lower_triangle_inverse(block_size, LR_pt+(k*MAT_DIM+k)*block_size, temp);	
		barrier(nc);
		for(int i = k+1; i < num_block; i++){
			if(i%nc == cid){
				elem_t temp_mult[block_size][block_size];
				full_transposed_matmul(block_size, 1, 1, 1, MAT_DIM, block_size, block_size, LR_pt+block_size*(i*MAT_DIM+k), (elem_t*) temp, (elem_t*) temp_mult, false);	
				copy_matrix(block_size, block_size, MAT_DIM, (elem_t*) temp_mult, LR_pt+block_size*(i*MAT_DIM+k));
			}
		}
		barrier(nc);
		...
		for(int j = k+1; j < num_block; j++){
			if(j%nc == cid)
				full_transposed_matmul(block_size, (num_block-j), 1, 1, MAT_DIM, MAT_DIM, MAT_DIM, LR_pt+block_size*(j*MAT_DIM+k), LR_pt+block_size*(j*MAT_DIM+k), LR_pt+block_size*(j*MAT_DIM+j), true);
		}
		barrier(nc);
  }
