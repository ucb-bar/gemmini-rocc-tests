...
static full_t in_A0[MAT_DIM_I][MAT_DIM_K] row_align(MAX_BLOCK_LEN) = {1};
static full_t in_B0[MAT_DIM_K][MAT_DIM_J] row_align(MAX_BLOCK_LEN) = {1};
//static elem_t full_C[MAT_DIM_I][MAT_DIM_J] row_align(1);
//static ACC_T bias[MAT_DIM_I][MAT_DIM_J] row_align_acc(1) = {0};
  ...
  for(int j = 0; j < nc; j++){
	if(j==cid)	{
		 tiled_matmul_auto(MAT_DIM, MAT_DIM, MAT_DIM, 
				&in_A[cid], &in_B[cid], NULL, &Out[cid],
  ...
  //barrier(nc);
	...
	//printf("thread: %d, loop: %d \n", cid, j);
	 if(cid == 0){
		 ...
		 full_matshift(in_A0, Out0, 1);
		 full_matshift(in_B0, Out0, 1);
	 }
