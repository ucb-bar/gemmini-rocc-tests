...
A = [[0 for x in range(MAT_DIM_K)] for y in range(MAT_DIM_I)]
B = [[0 for x in range(MAT_DIM_J)] for y in range(MAT_DIM_K)]
D = [[0 for x in range(MAT_DIM_J)] for y in range(MAT_DIM_I)]

total_diag_block = int(MAT_DIM_I/DIM)
for i in range(total_diag_block):
    for inner_i in range(DIM):
        A[i*DIM+inner_i]
