...
int main() {
  ...
  static elem_t result_A[DIM_ROWS][DIM] row_align(1);
  static elem_t result_B[DIM][DIM] row_align(1);
  static elem_t result_D[DIM_ROWS][DIM] row_align(1);

  static elem_t gold_A[DIM_ROWS][DIM];
  static elem_t gold_B[DIM][DIM];
  static elem_t gold_D[DIM_ROWS][DIM];
  ...
  gemmini_mvin(IDENTITY, IDENTITY1_addr);
  gemmini_mvin(IDENTITY, IDENTITY2_addr);
  gemmini_extended_mvin(IDENTITY, A_addr, DIM, DIM_ROWS);
  gemmini_mvin(IDENTITY, B_addr);
  gemmini_extended_mvin(IDENTITY, D_addr, DIM, DIM_ROWS);
  ...
  for (int i = 0; i < a_additions; i++) {
    ...
    gemmini_extended_preload(IDENTITY1_addr, A_addr, DIM, DIM_ROWS, DIM, DIM_ROWS);
    gemmini_extended_compute_preloaded(A_addr, IDENTITY2_addr, DIM, DIM_ROWS, DIM, DIM);
  }
  ...
  for (int i = 0; i < b_additions; i++) {
    ...
    gemmini_extended_preload(IDENTITY1_addr, B_addr, DIM, DIM_ROWS, DIM, DIM_ROWS);
    gemmini_extended_compute_preloaded(IDENTITY2_addr, B_addr, DIM, DIM_ROWS, DIM, DIM);
  }
  ...
  for (int i = 0; i < d_additions; i++) {
    ...
    gemmini_extended_preload(D_addr, D_addr, DIM, DIM_ROWS, DIM, DIM_ROWS);
    gemmini_extended_compute_preloaded(IDENTITY1_addr, IDENTITY2_addr, DIM, DIM_ROWS, DIM, DIM);
  }
  ...
  gemmini_extended_mvout(result_A, A_addr, DIM_ROWS, DIM);
  gemmini_extended_mvout(result_B, B_addr, DIM_ROWS, DIM);
  gemmini_extended_mvout(result_D, D_addr, DIM_ROWS, DIM);
  ...
  if (!is_equal_os(result_A, gold_A)) {
    printf("A:\n");
  ...
  if (!is_equal_os(result_B, gold_B)) {
    printf("B:\n");
  ...
  if (!is_equal_os(result_D, gold_D)) {
    printf("D:\n");
