...
void lower_triangle_inverse(elem_t* A, elem_t* M){
	for(int i = 0; i < block_dim; i++){
...
void small_block_left_chol(elem_t* L, int small_dim){
...
void *panel_update1(void *args){
	...
	if(cpu_id == 0){
		full_left_chol(LL_pt+block_dim*(MAT_DIM*k+k)); // cannot overlap first one
		lower_triangle_inverse(LL_pt+(k*MAT_DIM+k)*block_dim, chol_args->temp);
	}
	else{
...
void *panel_update2(void *args){
	...
	if(cpu_id == 0){
		int i = k+1;
		full_transposed_matmul(1, 1, 1, MAT_DIM, block_dim, MAT_DIM, LL_pt+block_dim*(i*MAT_DIM+k), chol_args->temp, LL_pt+block_dim*(i*MAT_DIM+k), false);	
		full_transposed_matmul(1, 1, i, MAT_DIM, MAT_DIM, MAT_DIM, LL_pt+block_dim*(i*MAT_DIM), LL_pt+block_dim*(i*MAT_DIM), LL_pt+block_dim*i*(MAT_DIM+1), true);	
	}
	else{
		int end = k < num_block - 2 ? num_block - 1 : num_block;
...
//		for(int i = start; i < num_block; i++){
			if(length > 0) full_transposed_matmul(length, 1, 1, MAT_DIM, block_dim, MAT_DIM, LL_pt+block_dim*(start*MAT_DIM+k), chol_args->temp, LL_pt+block_dim*(start*MAT_DIM+k), false);	
	//	}
	}
}
...
int main() {
    ...
    uint64_t cpu_end = 0;
    int small_block_num = block_dim/16;
...
// pthread_create(&thread[1], NULL, print_message, NULL);
 ...
 cpu_start = read_cycles();
  ...
  for(int k = 0; k < num_block; k++){
	...
	if(k == 0){
		small_block_left_chol(LL_pt, block_dim/small_block_num);
		//full_left_chol(LL_pt);
		lower_triangle_inverse(LL_pt, (elem_t*) temp_inv);
	}
	else{
		for(int i = 0; i < num_proc; i++){
			matmul_args[i].k = k;
