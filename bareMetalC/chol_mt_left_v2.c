...
static elem_t LL_mt[MAT_DIM][MAT_DIM] row_align(1) = {0};	
 ...
 for(int i = 0; i < num_block; i++)
	 for(int j = 0; j < num_block; j++)
		 for(int ii = 0; ii < block_size; ii++){
			 for(int jj = 0; jj < block_size; jj++){
				 if(j > i)
					 LL_mt[i*block_size+ii][j*block_size+jj] = 0;
				 else
					 LL_mt[i*block_size+ii][j*block_size+jj] = in_A[i*block_size+ii][j*block_size+jj];
			 }
		 }
  ...
  for(int k = 0; k < num_block; k++){
	  elem_t temp_inv[block_dim][block_dim] = {0};
 	   if(cid == 0) {
			//if(k == 0) 
				full_left_chol(LL_pt+block_size*(MAT_DIM*k+k)); // cannot overlap first one
			lower_triangle_inverse(LL_pt+(k*MAT_DIM+k)*block_dim, temp_inv);
		}
		//int end = k < num_block - 3 ? (num_block+k)/2 : num_block;
		int end = k < num_block - 2 ? num_block - 1 : num_block;
		if(k > 0 && cid != 0){
			for(int i = k+1; i < end; i++)
				full_transposed_matmul(1, 1, k, MAT_DIM, MAT_DIM, MAT_DIM, LL_pt+block_dim*(i*MAT_DIM), LL_pt+block_dim*(k*MAT_DIM), LL_pt+block_dim*(i*MAT_DIM+k), true);
		}
		barrier(nc);
		...
		if(cid != 0){
			if(end != num_block && k > 0)
				for(int i = end; i < num_block; i++)
					full_transposed_matmul(1, 1, k, MAT_DIM, MAT_DIM, MAT_DIM, LL_pt+block_dim*(i*MAT_DIM), LL_pt+block_dim*(k*MAT_DIM), LL_pt+block_dim*(i*MAT_DIM+k), true);
			lower_triangle_inverse(LL_pt+(k*MAT_DIM+k)*block_dim, temp_inv); // not needed for linux
			for(int i = k+2; i < num_block; i++){
				bool skip_B = (i != k+2);
				full_transposed_matmul(1, 1, 1, MAT_DIM, block_dim, MAT_DIM, LL_pt+block_dim*(i*MAT_DIM+k), skip_B ? NULL : (elem_t*)temp_inv, LL_pt+block_dim*(i*MAT_DIM+k), false);	
			}
		}
		if(cid == 0){
			if(num_block > k + 1){
				for(int i = k+1; i < k+2; i++){
					//bool skip_B = (i != k+1);
					full_transposed_matmul(1, 1, 1, MAT_DIM, block_dim, MAT_DIM, LL_pt+block_dim*(i*MAT_DIM+k), (elem_t*)temp_inv, LL_pt+block_dim*(i*MAT_DIM+k), false);	
				}
			}
			if(k != num_block - 1) full_transposed_matmul(1, 1, k+1, MAT_DIM, MAT_DIM, MAT_DIM, LL_pt+block_size*((1+k)*MAT_DIM), LL_pt+block_size*((1+k)*MAT_DIM), LL_pt+block_size*(k+1)*(MAT_DIM+1), true);	
		}
		barrier(nc);
  }
//  barrier(nc);
  ...
  if(cid == 0){
	  if (!full_is_equal(LL_mt, gold_L)) {
			printf("C:\n");
			full_printMatrix(LL_mt);
