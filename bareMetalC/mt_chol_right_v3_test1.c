...
void small_block_right_chol(int block_dim, elem_t* L, int small_dim){
...
void *panel_update1(void *args){
	...
	if(length > 0) full_transposed_matmul(block_dim, length, 1, 1, MAT_DIM, block_dim, MAT_DIM, L+block_dim*(i*MAT_DIM+k), chol_args->temp, L+block_dim*(i*MAT_DIM+k), false);				
...
void *panel_update2(void *args){
	...
	if(cid == 0 && k < num_block - 1){
	   ...
	   if(block_dim > 32) small_block_right_chol(block_dim, LR_pt+block_dim*(MAT_DIM+1)*(k+1), small_block_dim);
	   else full_right_chol(block_dim, LR_pt + block_dim * (MAT_DIM + 1)*(k + 1));
	   lower_triangle_inverse(block_dim, LR_pt+(MAT_DIM+1)+(k+1)*block_dim, chol_args->temp);	
	}
}
...
// pthread_create(&thread[1], NULL, print_message, NULL);
 ...
 int block_dim, small_block_dim, num_block;
/*
 block_dim = 32;
 ...
 cpu_start = read_cycles();
  ...
  for(int k = 0; k < num_block-1; k++){
	...
	if(k == 0){
	   small_block_right_chol(block_dim, LR_pt, small_block_dim);
 ...
 cpu_start = read_cycles();
  ...
  for(int k = 0; k < num_block-1; k++){
	if(k == 0)
	   small_block_right_chol(block_dim, LR_pt, small_block_dim);
	elem_t temp[block_dim][block_dim];// = {0};
	if(k == 0) lower_triangle_inverse(block_dim, LR_pt+(k*MAT_DIM+k)*block_dim, temp);
	//barrier
	for(int i = 0; i < num_proc; i++){
		matmul_args[i].k = k;
		matmul_args[i].block_dim = block_dim;
