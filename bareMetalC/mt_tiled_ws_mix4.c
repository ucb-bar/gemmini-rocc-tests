...
void thread_entry(int cid, int nc)
{
  ...
  //barrier(nc);
	 ...
	 elem_t* B = (elem_t*) in_B0 + (MAT_DIM/2)*(cid%2);
	 elem_t* C = (elem_t*) Out0 + (MAT_DIM/2)*(cid%2) + MAT_DIM_J*(MAT_DIM/2)*(cid/2);
	 acc_t * D = (acc_t*) bias0 + (MAT_DIM/2)*(cid%2) + MAT_DIM_J*(MAT_DIM/2)*(cid/2);
  ...
  for(int j = 0; j < nc; j++){
	...
	if(j == cid){	
		 tiled_matmul_auto(MAT_DIM/2, MAT_DIM/2, MAT_DIM, 
            ...
            A_TRANSPOSE, B_TRANSPOSE, SKIP_A, (cid == 0 || cid == 3) ? false : SKIP_B, 0,
            WS);
	}
  }
  ...
  for(int j = 0; j < nc; j++){
	...
	if(j == cid){	
		 tiled_matmul_auto(MAT_DIM/2, MAT_DIM/2, MAT_DIM, 
            ...
            MVIN_SCALE_IDENTITY, MVIN_SCALE_IDENTITY, MVIN_SCALE_IDENTITY,
            NO_ACTIVATION, ACC_SCALE_IDENTITY, 0, REPEATING_BIAS,
            A_TRANSPOSE, B_TRANSPOSE, SKIP_A, (cid == 1 || cid == 2) ? false : SKIP_B, 0,
            WS);
	}
  }
