...
void full_matmul_resadd(elem_t A[MAT_DIM_I][MAT_DIM_K], elem_t B[MAT_DIM_K][MAT_DIM_J], ACC_T D[MAT_DIM_I][MAT_DIM_J], ACC_T E[MAT_DIM_I][MAT_DIM_J], full_t C_full[MAT_DIM_I][MAT_DIM_J]) {
  for (size_t r = 0; r < MAT_DIM_I; r++)
    for (size_t c = 0; c < MAT_DIM_J; c++) {
      C_full[r][c] = D[r][c] + E[r][c];
      for (size_t k = 0; k < MAT_DIM_K; k++)
        C_full[r][c] += A[r][k]*B[k][c];
    }
}
...
void full_subtractMatrix(elem_t A[MAT_DIM_I][MAT_DIM_J], elem_t B[MAT_DIM_I][MAT_DIM_J], elem_t C[MAT_DIM_I][MAT_DIM_J]) {
  for (size_t i = 0; i < MAT_DIM_I; ++i) {
    for (size_t j = 0; j < MAT_DIM_J; ++j)
      C[i][j] = A[i][j]-B[i][j];
  }
...
int main() {
    ...
    static ACC_T full_E[MAT_DIM_I][MAT_DIM_J] row_align_acc(1);

    static full_t gold_full[MAT_DIM_I][MAT_DIM_J];
    static elem_t gold[MAT_DIM_I][MAT_DIM_J];

	static elem_t gold_out_diff[MAT_DIM_I][MAT_DIM_J];
...
//    full_matmul(full_A, full_B, full_D, gold_full);
    ...
    tiled_matmul_resadd_auto(MAT_DIM_I, MAT_DIM_J, MAT_DIM_K,
			...
			NO_BIAS ? NULL : &full_D[0][0], &full_E[0][0], (elem_t*)full_C,
            MAT_DIM_K, MAT_DIM_J, MAT_DIM_J, MAT_DIM_J, MAT_DIM_J,
            MVIN_SCALE_IDENTITY, MVIN_SCALE_IDENTITY, MVIN_SCALE_IDENTITY, MVIN_SCALE_IDENTITY,
    ...
    if (!full_is_equal(full_C, gold)) {
      ...
      printf("\n");
      full_subtractMatrix(full_C, gold, gold_out_diff);
      printf("Diff:\n");
      full_printMatrix(gold_out_diff);
      printf("\n");
