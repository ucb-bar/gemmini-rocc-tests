...
static elem_t in_B1[MAT_DIM_K][MAT_DIM_J] row_align(2) = {1};
static elem_t Out1[MAT_DIM_I][MAT_DIM_J] row_align(2) = {1};
static ACC_T bias1[MAT_DIM_I][MAT_DIM_J] row_align_acc(2) = {1};
...
void thread_entry(int cid, int nc)
{
  ...
  if(cid == 0){
	...
	gemmini_flush(0);
	 barrier(nc);
	 A = (elem_t*) in_A0 + MAT_DIM_J*(MAT_DIM_I/4)*cid;
	 B = (elem_t*) in_B0;// + (MAT_DIM/2)*(cid%2);
	 C = (elem_t*) Out0 + MAT_DIM_K*(MAT_DIM_I/4)*cid;
	 D = (acc_t*) bias0 + MAT_DIM_K*(MAT_DIM_I/4)*cid;
  ...
  for (int i = 0; i < nc; i++) {
  	...
  	gemmini_flush(0);
	 A = (elem_t*) Out0 + MAT_DIM_K*(MAT_DIM_I/4)*cid;
	 B = (elem_t*) in_B1;// + (MAT_DIM/2)*(cid%2);
	 C = (elem_t*) Out1 + MAT_DIM_J*(MAT_DIM_I/4)*cid;
	 D = (acc_t*) bias1 + MAT_DIM_J*(MAT_DIM_I/4)*cid;
