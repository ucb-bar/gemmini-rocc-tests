...
#include "musae_facebook_edges.h"
...
int main() {
  ...
  elem_t *_B = *B_ptr;
  elem_t *_C = *C_ptr;
  printf("dimension I: %d, J:%d \n", dim_I, dim_J);
  printf("nonzero data: %d \n", *(S_indptr + dim_I));	
//normal dense matmul
    printf("Starting dense matmul \n");
    uint64_t dense_start = read_cycles();
    tiled_matmul_auto(MAT_DIM_I, MAT_DIM_J, MAT_DIM_K,
            (elem_t*)_A, (elem_t*)_B, NO_BIAS ? NULL : &full_D[0][0], (elem_t*)_C,
            MAT_DIM_K, MAT_DIM_J, MAT_DIM_J, MAT_DIM_J,
            MVIN_SCALE_ONE, MVIN_SCALE_ONE, MVIN_SCALE_ONE,
            NO_ACTIVATION, 0, 0, false,
            WS);
    uint64_t dense_end = read_cycles();
...
//  full_matdot(gold_dense, S, gold);
    ...
    uint64_t sddmm_start = read_cycles();
    tiled_matmul_auto_sddmm(MAT_DIM_I, MAT_DIM_J, MAT_DIM_K,
            (elem_t*)_A, (elem_t*)_B, NO_BIAS ? NULL : &full_D[0][0], (elem_t*)_C, (int*) S_indptr, (int*) S_index, (int*) S_indptr_j, (int*) S_index_j,
            MAT_DIM_K, MAT_DIM_J, MAT_DIM_J, MAT_DIM_J,
            MVIN_SCALE_ONE, MVIN_SCALE_ONE, MVIN_SCALE_ONE,
            NO_ACTIVATION, 0, 0, false,
            WS);
    uint64_t sddmm_end = read_cycles();
