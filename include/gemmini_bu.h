  ...
  for (size_t j = 0; j < J; j++) {
    ...
    for (size_t k = 0; k < K; k++) {
	 ...
	 C_sp_addr = D_sp_addr;
       ...
       for(size_t i = 1; i < I; i++){ // may need to unroll this loop
	  const size_t A_rows = DIM - (i == I - 1 ? pad_I : 0);
	  gemmini_extended_preload(GARBAGE_ADDR, C_sp_addr + i*J*DIM, B_cols, B_rows, C_cols, DIM);
	  gemmini_extended_compute_accumulated(A_sp_addr + i*K*DIM, GARBAGE_ADDR, A_cols, A_rows, DIM, DIM); 
       }
    }
 }
 ...
 // Move-out C (merged with D mvin)
    ...
    for (size_t i = 0; i < I; i++) {
       ...
       for (size_t j = 0; j < J; j++) {
          const acc_t * const D_dram_addr = (acc_t *)D + (bias_row * D_row_stride + j)*DIM;
          const size_t C_cols = DIM - (j == J - 1 ? pad_J : 0);
          gemmini_extended_mvin(D_dram_addr, C_sp_addr+DIM*j, C_cols, C_rows); //merge loops with bias mv-in
...
//	if(output!=NULL) gemmini_extended_mvout(output + (b * out_dim * out_dim)*out_channels + och, C_sp_addr, DIM, 0);
    ...
    else{ //no need to move-in weight in inner loop
 if(odims > DIM){
   for (int b = 0; b < batches; b++){
        for (int och = 0; och < ochs; och += DIM) {
 	   const uint32_t B_sp_addr = B_sp_addr_start + (och / DIM) * kchs;// + kch;	 
...
//    gemmini_fence(); // TODO fix ROB to get rid of this requirement
    for (int b = 0; b < batches; b++) {
     ...
     if(output!=NULL)     gemmini_extended_config_st(out_channels * sizeof(elem_t), pool_stride, pool_size, pool_out_dim, porows, pocols, orows, ocols, pupad, plpad);
...
void tiled_conv_auto_original(
   ...
   const int weight_bank = 2; //allocate 2 banks
   const bool no_pool = pool_stride == 0 || (pool_stride == 1 && pool_size == 1 && pool_padding == 0);
//    const bool no_1d = pool_stride == 0 && pool_size == 0;
