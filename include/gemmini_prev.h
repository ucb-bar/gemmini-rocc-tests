...
static void sp_tiled_matmul_ws(const elem_t * A, const elem_t * B,
  ...
  uint64_t start, end, mvin_cycle, matmul_cycle, mvout_cycle;
  start = read_cyclesh();
  gemmini_fence();
  // Move-in D
  if (D != NULL && !no_bias) {
  ...
  if (C != NULL) {
    for (size_t i = 0; i < I; i++) {
      for (size_t j = 0; j < J; j+= D_blocks) {
        ...
        gemmini_extended_mvout(C_dram_addr, C_sp_addr, cols, C_rows);
      }
    }
  }
  gemmini_fence();
  end = read_cyclesh();
  mvout_cycle = end - start;
  printf("mvin: %llu, matmul: %llu, mvout: %llu \n", mvin_cycle, matmul_cycle, mvout_cycle);

}

static void sp_tiled_matmul_transpose_ws_share(const elem_t * A,// const elem_t * B,
        const acc_t * D, elem_t * C,
        scale_t A_scale_factor, scale_acc_t D_scale_factor,
        size_t I, size_t J, size_t K, size_t pad_I, size_t pad_J, size_t pad_K,
        size_t A_row_stride, size_t D_row_stride, size_t C_row_stride,
        bool no_bias, bool repeating_bias) {

  const uint32_t A_sp_addr_start = 0;
  const uint32_t B_sp_addr_start = 0;//BANK_NUM * BANK_ROWS - K * J * DIM;
  const uint32_t D_sp_addr_start = 1 << (ADDR_LEN-1);
  const uint32_t C_sp_addr_start = 3 << (ADDR_LEN-2);
...
static void sp_tiled_matmul_transpose_ws(const elem_t * A, const elem_t * B,
        const acc_t * D, elem_t * C,
        scale_t A_scale_factor, scale_t B_scale_factor, scale_acc_t D_scale_factor,
        size_t I, size_t J, size_t K, size_t pad_I, size_t pad_J, size_t pad_K,
        size_t A_row_stride, size_t B_row_stride, size_t D_row_stride, size_t C_row_stride,
  ...
  if(B!=NULL){
	  gemmini_extended_config_ld(B_row_stride * sizeof(elem_t), B_scale_factor);
	  for (size_t j = 0; j < J; j++) {
		for (size_t k = 0; k < K; k += B_blocks) {//K is stride
...
static void tiled_matmul_transpose_outer(size_t dim_I, size_t dim_J, size_t dim_K,
        const elem_t* A, const elem_t* B,
        const acc_t * D, elem_t* C,
        size_t stride_A, size_t stride_B, size_t stride_D, size_t stride_C,
        scale_t A_scale_factor, scale_t B_scale_factor, scale_acc_t D_scale_factor,
      ...
      for (size_t k0 = 0; k0 < K0; k0++) {
        ...
        if (dataflow == OUTPUT_STATIONARY) {
          sp_tiled_matmul_os(A + i0*tile_I*DIM*stride_A + k0*tile_K*DIM,
              ...
              stride_A, stride_B, stride_D, stride_C,
              no_bias, repeating_bias);
        } else { //for now, just B_transpose(JxK)
          sp_tiled_matmul_transpose_ws(A + i0*tile_I*DIM*stride_A + k0*tile_K*DIM,
              (B == NULL) ? NULL : B + k0*tile_K*DIM + j0*tile_J*DIM*stride_B,
              //B + k0*tile_K*DIM*stride_B + j0*tile_J*DIM,
              pre, out,
              A_scale_factor, B_scale_factor, D_scale_factor,
              I, J, K,
...
static void tiled_matmul_transpose_outer_share(size_t dim_I, size_t dim_J, size_t dim_K,
        const elem_t* A, //const elem_t* B,
        const acc_t * D, elem_t* C,
        size_t stride_A, size_t stride_D, size_t stride_C,
        scale_t A_scale_factor, scale_acc_t D_scale_factor,
        size_t tile_I, size_t tile_J, size_t tile_K,
        int act, acc_scale_t scale, size_t relu6_shift, bool repeating_bias,
		  bool A_transpose, bool B_transpose,
    ...
    for (size_t j0 = 0; j0 < J0; j0++)
      for (size_t k0 = 0; k0 < K0; k0++) {
        ...
        const size_t pad_K = k0 == K0-1 ? padding_K : 0;
         ...
         sp_tiled_matmul_transpose_ws_share(A + i0*tile_I*DIM*stride_A + k0*tile_K*DIM,
              //B + k0*tile_K*DIM + j0*tile_J*DIM*stride_B,
              pre, out,
              A_scale_factor, D_scale_factor,
              I, J, K,
              pad_I, pad_J, pad_K,
              stride_A, stride_D, stride_C,
              no_bias, repeating_bias);
      }
...
void tiled_matmul_transpose_share(size_t dim_I, size_t dim_J, size_t dim_K,
        const elem_t* A,// const elem_t* B,
        const acc_t * D, elem_t* C,
        size_t stride_A, size_t stride_D, size_t stride_C,
        scale_t A_scale_factor, scale_acc_t D_scale_factor,
...
//      (tile_K * tile_J * DIM);    // Rows to store B
  ...
  // Run a tiled matrix multiplication on either Gemmini or the CPU
  if (tiled_matmul_type == OS || tiled_matmul_type == WS) {
      tiled_matmul_transpose_outer_share(dim_I, dim_J, dim_K,
              A, D, C,
              stride_A, stride_D, stride_C,
              A_scale_factor, D_scale_factor,
              tile_I, tile_J, tile_K,
...
void tiled_matmul_transpose(size_t dim_I, size_t dim_J, size_t dim_K,
        const elem_t* A, const elem_t* B,
        const acc_t * D, elem_t* C,
        size_t stride_A, size_t stride_B, size_t stride_D, size_t stride_C,
        scale_t A_scale_factor, scale_t B_scale_factor, scale_acc_t D_scale_factor,
  ...
  // Run a tiled matrix multiplication on either Gemmini or the CPU
  if (tiled_matmul_type == OS || tiled_matmul_type == WS) {
      tiled_matmul_transpose_outer(dim_I, dim_J, dim_K,
              A, B, D, C,
              stride_A, stride_B, stride_D, stride_C,
              A_scale_factor, B_scale_factor, D_scale_factor,
              tile_I, tile_J, tile_K,
...
// This function runs a transposed tiled matrix multiplication
void tiled_matmul_transpose_auto(size_t dim_I, size_t dim_J, size_t dim_K,
        const elem_t* A, const elem_t* B,
        const acc_t * D, elem_t* C,
        size_t stride_A, size_t stride_B, size_t stride_D, size_t stride_C,
        scale_t A_scale_factor, scale_t B_scale_factor, scale_acc_t D_scale_factor,
...
#define max_tile_k (mats_in_partition / max_tile_i_j)
    ...
    tiled_matmul_transpose(dim_I, dim_J, dim_K,
        A, B, D, C,
        stride_A, stride_B, stride_D, stride_C,
        A_scale_factor, B_scale_factor, D_scale_factor,
        act, scale, relu6_shift, repeating_bias,
...
// This function runs a transposed tiled matrix multiplication
void tiled_matmul_transpose_auto_share(size_t dim_I, size_t dim_J, size_t dim_K,
        const elem_t* A, //const elem_t* B,
        const acc_t * D, elem_t* C,
        size_t stride_A, size_t stride_D, size_t stride_C,
        scale_t A_scale_factor, scale_acc_t D_scale_factor,
        int act, acc_scale_t scale, size_t relu6_shift, bool repeating_bias,
		  bool A_transpose, bool B_transpose,
        enum tiled_matmul_type_t tiled_matmul_type) {
#define partition_rows (BANK_NUM * BANK_ROWS)
#define mats_in_partition (partition_rows / DIM)
#define mats_in_acc (ACC_ROWS / DIM)
#define max_tile_i_j ((size_t)sqrt(mats_in_acc))
#define max_tile_k (mats_in_partition / max_tile_i_j)
    ...
    tiled_matmul_transpose_share(dim_I, dim_J, dim_K,
        A, D, C,
        stride_A, stride_D, stride_C,
        A_scale_factor, D_scale_factor,
        act, scale, relu6_shift, repeating_bias,
