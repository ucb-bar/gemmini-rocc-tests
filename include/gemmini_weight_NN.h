...
*/
  ...
  // Move-in B(w/o block mvin)
  gemmini_extended_config_ld(B_row_stride * sizeof(elem_t), B_scale_factor);
  for (size_t j = 0; j < J; j += 1) {
     const size_t cols = DIM - (j + 1 >= J ? pad_J : 0); 
    for (size_t k = 0; k < K; k++) {
      const elem_t * const B_dram_addr = B + (k*B_row_stride + j)*DIM;
      const uint32_t B_sp_addr = B_sp_addr_start + (k*J + j)*DIM;
      const size_t rows = DIM - (k == K-1 ? pad_K : 0);
      gemmini_extended_mvin(B_dram_addr, B_sp_addr, cols, rows);
        ...
        const uint32_t C_sp_addr = C_sp_addr_start + (j)*DIM;
       ...
       if (no_bias_new_matrix) {
        ...
        const size_t A_cols = DIM - (k == K - 1 ? pad_K : 0);
        const size_t B_cols = cols;//DIM - (j == J - 1 ? pad_J : 0);
        const size_t B_rows = DIM - (k == K - 1 ? pad_K : 0);
        const size_t C_cols = cols;//DIM - (j == J - 1 ? pad_J : 0);

        gemmini_extended_preload(B_sp_addr, out_sp_addr, B_cols, B_rows, C_cols, DIM);
          gemmini_extended_compute_preloaded(A_sp_addr, GARBAGE_ADDR, A_cols, DIM, DIM, DIM);
 
         gemmini_extended_preload(GARBAGE_ADDR, out_sp_addr+DIM*J, B_cols, B_rows, C_cols, DIM);
           gemmini_extended_compute_accumulated(A_sp_addr+K*DIM, GARBAGE_ADDR, A_cols, DIM, DIM, DIM);
         gemmini_extended_preload(GARBAGE_ADDR, out_sp_addr+DIM*J*2, B_cols, B_rows, C_cols, DIM);
           gemmini_extended_compute_accumulated(A_sp_addr+K*DIM*2, GARBAGE_ADDR, A_cols, DIM, DIM, DIM);
         gemmini_extended_preload(GARBAGE_ADDR, out_sp_addr+DIM*J*3, B_cols, B_rows, C_cols, DIM);
           gemmini_extended_compute_accumulated(A_sp_addr+K*DIM*3, GARBAGE_ADDR, A_cols, DIM, DIM, DIM);

         gemmini_extended_preload(GARBAGE_ADDR, out_sp_addr+DIM*J*4, B_cols, B_rows, C_cols, DIM);
           gemmini_extended_compute_accumulated(A_sp_addr+K*DIM*4, GARBAGE_ADDR, A_cols, DIM, DIM, DIM);
         gemmini_extended_preload(GARBAGE_ADDR, out_sp_addr+DIM*J*5, B_cols, B_rows, C_cols, DIM);
           gemmini_extended_compute_accumulated(A_sp_addr+K*DIM*5, GARBAGE_ADDR, A_cols, DIM, DIM, DIM);
         gemmini_extended_preload(GARBAGE_ADDR, out_sp_addr+DIM*J*6, B_cols, B_rows, C_cols, DIM);
           gemmini_extended_compute_accumulated(A_sp_addr+K*DIM*6, GARBAGE_ADDR, A_cols, DIM, DIM, DIM);
	gemmini_extended_preload(GARBAGE_ADDR, out_sp_addr+DIM*J*7, B_cols, B_rows, C_cols, DIM-pad_I);
           gemmini_extended_compute_accumulated(A_sp_addr+K*DIM*7, GARBAGE_ADDR, A_cols, DIM-pad_I, DIM, DIM);
...
static void tiled_matmul_outer_fitC(size_t dim_I, size_t dim_J, size_t dim_K,
  ...
  for (size_t k = 0; k < K; k += A_blocks) {
      const size_t blocks = k + A_blocks <= K ? A_blocks : K-k;
      const size_t cols = blocks * DIM - (k + blocks >= K ? pad_K : 0); 
    for (size_t i = 0; i < I; i=i+4) {
      const elem_t * const A_dram_addr = (A + i0*tile_I*DIM*stride_A) + (i * stride_A + k)*DIM;
      const uint32_t A_sp_addr = A_sp_addr_start + (i*K + k)*DIM;
      const size_t rows = DIM - (i == I-1 ? pad_I : 0);
      gemmini_extended_mvin(A_dram_addr, A_sp_addr, cols, rows);
      gemmini_extended_mvin(A_dram_addr+stride_A*DIM, A_sp_addr+K*DIM, cols, rows);
      gemmini_extended_mvin(A_dram_addr+2*stride_A*DIM, A_sp_addr+2*K*DIM, cols, rows);
      gemmini_extended_mvin(A_dram_addr+3*stride_A*DIM, A_sp_addr+3*K*DIM, cols, rows);
    }
  }

    for (size_t j0 = 0; j0 < J0; j0++){
//      for (size_t k0 = 0; k0 < K0; k0++) {// no need for k dimension
...
//        }
        ...
        */} else {
          sp_tiled_matmul_ws_fitC(A_sp_addr_start,//A + i0*tile_I*DIM*stride_A,
              B + j0*tile_J*DIM,
              pre, out,
              A_scale_factor, B_scale_factor, D_scale_factor,
              I, J, K,
...
//    printf("mvin input count: %d \n", mvin_input_count);
//  printf("mvin weight cycle: %d, mvin weight count: %d \n", mvin_weight_cycle, mvin_weight_count);
  gemmini_fence();
}
...
static void tiled_matmul_outer(size_t dim_I, size_t dim_J, size_t dim_K,
  ...
  const bool no_bias = D == NULL;
/*
  uint64_t mvin_weight_cycle = 0; uint64_t mvin_weight_count = 0;
  uint64_t mvin_input_cycle = 0; uint64_t mvin_input_count = 0;
  uint64_t matmul_cycle = 0; uint64_t matmul_count = 0;
  uint64_t mvout_cycle = 0; uint64_t mvout_count = 0;
*/
  if (no_bias) {
    D = (acc_t*) 1; // Dummy address which isn't NULL
  }
  ...
  for (size_t i0 = 0; i0 < I0; i0++)
    for (size_t j0 = 0; j0 < J0; j0++)
	 ...
	 //     &mvin_weight_cycle, &mvin_weight_count, &mvin_input_cycle, &mvin_input_count,
	 //     &matmul_cycle, &matmul_count, &mvout_cycle, &mvout_count);
        }
      }
/*
  printf("mvin weight cycles: %d, mvin weight counts: %d, mvin input cycles: %d, mvin input counts: %d \n", mvin_weight_cycle, mvin_weight_count, mvin_input_cycle, mvin_input_count);
  printf("matmul cycles: %d, matmul counts: %d, mvout cycles: %d, mvout counts: %d \n", matmul_cycle, matmul_count, mvout_cycle, mvout_count);
*/
  gemmini_fence();
}
...
#undef max_tile_k
/*
#define mats_in_acc (ACC_ROWS / DIM) //how many DIMxDIM matrices can fit in acc
//#define max_tile_j 4
//#define max_tile_i 16//(mats_in_acc/max_tile_j)
#define partition_rows (BANK_NUM * BANK_ROWS / 2)
#define mats_in_partition (partition_rows / DIM)
#define max_tile_i mats_in_acc/max_tile_j
//#define max_tile_i_j ((size_t)sqrt(mats_in_acc))
#define max_tile_k (mats_in_partition / max_tile_j)

//#define max_tile_k dim_K/DIM //full load
 size_t A_partition_rows = BANK_NUM*BANK_ROWS/2;
if(dim_I > 2*dim_J && dim_I > 1000)
	A_partition_rows = 3*BANK_ROWS;
else if(dim_J > 2*dim_I && dim_J > 1000)
	A_partition_rows = 1*BANK_ROWS;
size_t  B_partition_rows=BANK_NUM*BANK_ROWS - A_partition_rows;
//#define B_partition_mats  B_partition_rows/DIM
//#define A_partition_mats  A_partition_rows/DIM
//#define max_tile_j B_partition_mats/max_tile_k
#define mats_in_acc (ACC_ROWS/DIM)
//#define max_tile_i A_partition_mats/max_tile_k

    const size_t dim_I_padded = (dim_I / DIM + (dim_I % DIM != 0)) * DIM;
    const size_t dim_J_padded = (dim_J / DIM + (dim_J % DIM != 0)) * DIM;
    const size_t dim_K_padded = (dim_K / DIM + (dim_K % DIM != 0)) * DIM;

size_t tile_K = dim_K_padded/DIM;
if(tile_K > 64)
	tile_K = 64; //for 2048 case
size_t max_tile_i = 16;
if(dim_I < dim_J) 
	max_tile_i = 8;
if(tile_K == 64)
	max_tile_i = 8;
size_t max_tile_j = 64/max_tile_i;

//    const size_t B_partition_mats = B_partition_rows/DIM;
//    const size_t A_partition_mats = A_partition_rows/DIM;
//    const size_t max_tile_j = B_partition_mats/tile_K;
//    const size_t max_tile_i = A_partition_mats/tile_K;
    size_t tile_I = dim_I_padded/DIM < max_tile_i ? dim_I_padded/DIM : max_tile_i;
    printf("tile_I: %d, tile_J: %d, tile_K: %d \n", tile_I, tile_J, tile_K);
//    printf("A_partition_mats: %d, B_partition_mats: %d, mats_in_acc: %d, max_tile_i: %d, max_tile_j: %d \n", A_partition_mats, B_partition_mats, mats_in_acc, max_tile_i, max_tile_j);
...
//    int kdims = krows*krows; 
	...
	const int turn = kchs%DIM == 0 ? kchs/DIM : kchs/DIM + 1;
	gemmini_extended_config_ex(WEIGHT_STATIONARY, act, 0, shift, relu6_shift, ocols, row_turn, 1, stride, kchs, row_left, 1, double_bank, triple_bank); //if want 2 banks for weight, last is 1
       ...
       gemmini_config_ld(0);
        for (int b = 0; b < batches; b++)
            for (int orow = 0; orow < orows; orow++)
                    for (int och = 0; och < ochs; och += DIM*4) {
//                        const int J = ochs - och > DIM ? DIM : ochs - och;

                        const uint32_t D_sp_addr = D_sp_addr_start + (och / DIM) * batches * odims + b * odims + orow * ocols;

                        gemmini_extended_mvin(bias + och,
                                D_sp_addr,
                                DIM, ocols);
                        gemmini_extended_mvin(bias + och + DIM,
                                D_sp_addr + odims*batches,
                                DIM, ocols);
                        gemmini_extended_mvin(bias + och + 2*DIM,
                                D_sp_addr + 2*batches*odims,
                                DIM, ocols);
                        gemmini_extended_mvin(bias + och + 3*DIM,
                                D_sp_addr + 3*batches*odims,
                                DIM, ocols);

                   }
   }

if(B_sp_addr_outer == 0){ //might have moved in outer loop
  // mvin weights
   gemmini_config_ld(out_channels * sizeof(elem_t));
    for (int och = 0; och < ochs; och += DIM) {
//        const int J = ochs - och > DIM ? DIM : ochs - och;
      for (int kch = 0; kch < kchs; kch += 4*DIM) {
//        const int K = kchs - kch > DIM ? DIM : kchs - kch;
                   const uint32_t B_sp_addr = B_sp_addr_start + (och / DIM) * kchs + kch;
		   elem_t * weight = weights + kch*out_channels + och;
                    gemmini_extended_mvin(weight,
                        B_sp_addr,
                        DIM, DIM);
                    gemmini_extended_mvin(weight + DIM*out_channels,
                        B_sp_addr+DIM,
                        DIM, DIM);   
                    gemmini_extended_mvin(weight + 2*DIM*out_channels,
                        B_sp_addr+2*DIM,
                        DIM, DIM);
                    gemmini_extended_mvin(weight + 3*DIM*out_channels,
                        B_sp_addr+3*DIM,
                        DIM, DIM);
        }
    }
...
//    gemmini_fence(); // TODO fix ROB to get rid of this requirement
    for (int b = 0; b < batches; b++) {
        for (int irow = 0; irow < irows; irow++) {
                for (int ich = 0; ich < ichs; ich += DIM*4) {
                    elem_t * in = input + (b*in_dim*in_dim + irow*in_dim) * in_channels + ich;
                    const uint32_t A_sp_addr = A_sp_addr_start + (ich / DIM) * bidims + b * idims + irow * icols;
                    gemmini_extended_mvin(in,
                            A_sp_addr,
                            DIM, icols);
                    gemmini_extended_mvin(in+DIM,
                            A_sp_addr+bidims,
                            DIM, icols);
                    gemmini_extended_mvin(in+2*DIM,
                            A_sp_addr+2*bidims,
                            DIM, icols);
                    gemmini_extended_mvin(in+3*DIM,
                            A_sp_addr+3*bidims,
                            DIM, icols); //loop unrolling
                }
       }
          for (int och = 0; och < ochs; och += DIM) {
//            const int J = ochs - och > DIM ? DIM : ochs - och;
   	    	const int C_sp_addr = C_sp_addr_start + (och / DIM) * batches * odims + b * odims;
...
//	        const int K = kchs - kch > DIM ? DIM : kchs - kch;
   ...
   if (output != NULL) {
	   ...
	   if(no_1d){
		...
		for(int b = 0; b < batches; b++)
			for(int och = 0; och < ochs; och += 4*DIM){
				//const int J = ochs - och > DIM ? DIM : ochs - och;
				...
				gemmini_extended_mvout(output + (b * out_dim * out_dim)*out_channels + och, C_sp_addr + batches*odims, DIM, 0);
				gemmini_extended_mvout(output + (b * out_dim * out_dim)*out_channels + och, C_sp_addr + 2*batches*odims, DIM, 0);
				gemmini_extended_mvout(output + (b * out_dim * out_dim)*out_channels + och, C_sp_addr + 3*batches*odims, DIM, 0);
			}	
		}
            ...
            for (int b = 0; b < batches; b++) {
                for (int poch = 0; poch < pochs; poch += DIM*4) {
//                    const int channels = poch + DIM >= pochs ? pochs - poch : DIM;

                    elem_t * pout = output + (b * pool_out_dim * pool_out_dim)*out_channels + poch;

                    const uint32_t C_sp_addr = C_sp_addr_start + (poch / DIM) * batches * odims + b * odims;

                    gemmini_extended_mvout(pout,
                            C_sp_addr,
                            DIM, 0);
                     gemmini_extended_mvout(pout + DIM,
                            C_sp_addr + batches*odims,
                            DIM, 0);
                     gemmini_extended_mvout(pout + 2*DIM,
                            C_sp_addr + 2*batches*odims,
                            DIM, 0);
                     gemmini_extended_mvout(pout + 3*DIM,
                            C_sp_addr + 3*batches*odims,
                            DIM, 0);
      ...
      for (int kch = 0; kch < kchs; kch += DIM) {
        ...
        for (int krow = 0; krow < krows; krow++)
            for (int kcol = 0; kcol < krows; kcol++){
                    const uint32_t B_sp_addr = B_sp_addr_start + (och / DIM) * kdims * kchs + kch*kdims + krow*krows*K + kcol*K;//krow * kcols * kchs + kcol * kchs + kch;
...
#endif
//    int double_buffer = weight_bank > 1 ? 1 : 0;
//    int triple_buffer = weight_bank > 2 ? 1 : 0;
...
void tiled_conv_ds(
...
#ifdef GEMMINI_ASSERTIONS
    {
        // Check that data will fit in scratchpad
        const int spad_rows_weight = tiled_conv_total_spad_rows(false, true,
            stride, batches, porows, pocols, pochs, 1, 1, kchs, pool_size, pool_stride);
        const int spad_rows_input = tiled_conv_total_spad_rows(false, false,
            stride, batches, porows, pocols, pochs, 1, 1, kchs, pool_size, pool_stride);
        const int acc_rows = tiled_conv_total_spad_rows(true, false,
            stride, batches, porows, pocols, pochs, 1, 1, kchs, pool_size, pool_stride);
  ...
  //          const int kchs_ = in_channels;
		  ...
		  for (int och = 0; och < pochs_; och += DIM) {
        		//const int J = pochs_ - och > DIM ? DIM : pochs_ - och;
      			...
      			for (int ich = 0; ich < in_channels; ich += DIM) {
        		    //const int K = kchs_ - ich > DIM ? DIM : kchs_ - ich;
                   	    const uint32_t B_sp_addr = B_sp_addr_start + (och / DIM) * in_channels + ich;//krow * kcols * kchs + kcol * kchs + kch;
	     		    gemmini_extended_mvin(weights + poch + ich * out_channels + och,
                      				B_sp_addr,
                        			DIM, DIM);
...
//			        const int kchs_ = in_channels - kch > kchs ? kchs : in_channels - kch;
                               ...
                               // const int icols_ = (ocols_ - plpad - prpad) * stride + kcols_ - 1;
                               // const int irows_ = (orows_ - pupad - pdpad) * stride + krows_ - 1;
				const int icols_ = (ocols_ - 1 - plpad - prpad) * stride + 1;//+ kcols_;
                                const int irows_ = (orows_ - 1 - pupad - pdpad) * stride + 1;//krows_;
/*
                                const int lpad = icol < 0 ? -icol : 0;
                                const int rpad = icol + icols_ > in_dim ? icol + icols_ - in_dim : 0;
                                const int upad = irow < 0 ? -irow : 0;
...
//                                    lpad, rpad, upad, dpad,
                                    ...
                                    input + (b*in_dim*in_dim + irow*in_dim + icol) * in_channels + kch,
				    0, //B_sp_addr (for weight outer most)
				    weights + kch * out_channels + poch,
...
void tiled_conv_auto_original1(
        int batch_size, int in_dim, int in_channels,
        int out_channels, int out_dim,
        int stride, int padding, int kernel_dim,
...
void tiled_conv_auto(
	...
	enum tiled_matmul_type_t tiled_conv_type, 
	int out_tile, int in_tile, int weight_bank) {
...
//    int kch_floor = (args[4]/DIM) + 1;
    while(acc_rows > ACC_ROWS){ //batch output channel, output dimension affects
	if(out_tile == 0){ //tile batch first
		if(args[0] != 1){
			args[0]--;
	...
	}else if(out_tile == 1){ //tile output dimension
		if(args[1] != 1){
			args[1]--;
			args[2]--;
		}else{
...
//    printf("batch: %d, out_dim: %d, out_channel: %d, in_channel: %d \n", args[0], args[1], args[3], args[4]);
	...
	}else if(in_tile == 1){ //input dimension
		if(args[1] != 1){
			args[1]--;
			args[2]--;
		}else{
...
void sp_tiled_resadd_neg(const size_t I, const size_t J,
        //const size_t A_shift,
        const elem_t * A, const elem_t * B, elem_t * C,
        size_t A_row_stride, size_t B_row_stride, size_t C_row_stride,
        bool relu) {
    ...
    const size_t rounded_up_J = (J / DIM + (J % DIM != 0)) * DIM;
    const size_t A_shift = 31; //multiply by 2
    ...
    // Store A values in accumulator
    // printf("Store A values in accumulator\n");
    gemmini_config_ex(WS, NO_ACTIVATION, A_shift, 0, 0);
    ...
    for (size_t i = 0; i < I; i += DIM) {
        for (size_t j = 0; j < J; j += DIM) {
            ...
            gemmini_extended_compute_preloaded(A_sp_addr, GARBAGE_ADDR, cols, rows, cols, rows);
        }
    }
...
void tiled_resadd_neg(const size_t I, const size_t J,
    ...
    for (int i = 0; i < I; i += tile_I) {
        for (int j = 0; j < J; j += tile_J) {
            ...
            sp_tiled_resadd_neg(I_tile, J_tile,
                    a, b, c,
                    J, J, J,
                    relu);
        }
...
void tiled_resadd_auto(const size_t I, const size_t J,
	...
	if(A_shift >= 0){
	        ...
	        tiled_resadd_neg(I, J, tile_I, tile_J,
	            A_shift, A, B, C,
        	    relu, matadd_type);	
	}
    } else {
