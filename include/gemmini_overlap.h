//overlapping - matmul_table row by row
...
#define gemmini_sddmm_mvout(dram_addr, spad_addr, row0, row1, row2, row3, cols, rows) \
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC, dram_addr, ((uint64_t)(rows) << (ADDR_LEN + 24)) | ((uint64_t)(cols) << ADDR_LEN+16) |((uint64_t)(row3) << (ADDR_LEN+3*DIM)) | ((uint64_t)(row2) << ADDR_LEN+2*DIM) | ((uint64_t)(row1) << ADDR_LEN+DIM) | ((uint64_t)(row0) << ADDR_LEN) | (uint64_t)(spad_addr), k_MVOUT)

#define gemmini_extended_mvout(dram_addr, spad_addr, cols, rows) \
  gemmini_sddmm_mvout(dram_addr, spad_addr, 0b1111, 0b1111, 0b1111, 0b1111, cols, rows)
/*
#define gemmini_extended_mvout(dram_addr, spad_addr, cols, rows) \
...
static void sp_tiled_matmul_ws_sddmm(const elem_t * A, const elem_t * B,
        const acc_t * D, elem_t * C, const int * S_indptr, const int * S_index, const int * S_indptr_j, const int * S_index_j,
        scale_t A_scale_factor, scale_t B_scale_factor, scale_acc_t D_scale_factor,
        size_t I, size_t J, size_t K, size_t pad_I, size_t pad_J, size_t pad_K,
        size_t A_row_stride, size_t B_row_stride, size_t D_row_stride, size_t C_row_stride,
	int tile_I_start, int tile_J_start, //for tiling starting point
        bool no_bias, bool repeating_bias) {
  ...
  const int A_blocks = 8;//K <= MAX_BLOCK_LEN ? K : MAX_BLOCK_LEN;
  const int B_blocks = J <= MAX_BLOCK_LEN ? J : MAX_BLOCK_LEN;
  const int D_blocks = J <= MAX_BLOCK_LEN_ACC ? J : MAX_BLOCK_LEN_ACC;

  int A_mvin[I+1]; //Todo: consider padding 
  A_mvin[0] = 0; //indirection for matmuls
  int B_mvin[J+1]; //block?
  B_mvin[0] = 0;
  int matmul_table[I][J];
  // Move-in D
  if (D != NULL && !no_bias) {
    const size_t D_stride = repeating_bias ? 0 : D_row_stride * sizeof(acc_t);
    gemmini_extended_config_ld(D_stride, D_scale_factor);
    ...
    for (size_t i = 0; i < I; i++) {
         const size_t rows = DIM - (i == I-1 ? pad_I : 0);
        const size_t bias_row = repeating_bias ? 0 : i;
         const uint32_t D_sp_addr_acc = D_sp_addr_start + (i*J)*DIM;
     ...
     for (size_t j = 0; j < J; j += D_blocks) {
        ...
        const size_t cols = blocks * DIM - (j + blocks >= J ? pad_J : 0);
        gemmini_extended_mvin(D_dram_addr, D_sp_addr_acc + j*DIM, cols, rows);
	for(size_t jj = 0; jj < D_blocks; jj++)
	    matmul_table[i][j + jj] = 0; //initialize array
      }
    }
  }

  size_t dim_I = I*DIM - pad_I; 
  gemmini_extended_config_ld(B_row_stride * sizeof(elem_t), B_scale_factor);
...
//  printf("B mvin : ");
    ...
    for (size_t j = 0; j < J; j += 1) {
     ...
     int end = *(S_indptr_j + position + cols);
     B_mvin[j+1] = B_mvin[j];
     if(start != end){
	for(size_t i = start; i < end; i++){
	   int index = *(S_index_j + i);
	   //i_save = i + 1;
	   //B_mvin[j+1] = B_mvin[j];
...
//	printf("j: %d, index: %d \n", j, index);
	...
	if(B_mvin[j+1]!=B_mvin[j])
	   for(size_t i = 0; i < dim_I; ){
	     int row_start = *(S_indptr + i + tile_I_start);
	     int row_end = *(S_indptr + i + tile_I_start + 1);
	     i++;
	     if(*(S_index+row_start) < tile_J_start + (j+1)*DIM && *(S_index+row_end-1) >= tile_J_start + j*DIM)	
		for(size_t ii = row_start; ii < row_end; ii++) {
		   int row_index = *(S_index + ii);
		   if(row_index >= tile_J_start + (j+1)*DIM) break;
		   if(row_index >= tile_J_start + j*DIM && row_index < tile_J_start + (j+1)*DIM){
			int row_block = i%DIM==0 ? (i/DIM) - 1 : (int)(i/DIM); //out of I
			matmul_table[row_block][j] = 1;
// printf("matmul_table[%d][%d], row: %d, row_index: %d  \n", row_block, j, i, row_index);
...
//	if(B_mvin[j] < B_mvin[j+1]){
	...
	for(size_t kk = 0; kk < 4; kk++)
	  gemmini_extended_mvin(B_dram_addr+B_mvin_col[j]+kk*B_row_stride*DIM, B_sp_addr+(kk*J+j)*DIM, cols, rows);
//	}
     }
   }
...
/*
  gemmini_extended_config_ld(A_row_stride * sizeof(elem_t), A_scale_factor);
   for (size_t i = 0; i < I; i++) {
      int position = i*DIM + tile_I_start;//current tile location for whole matrix
      const size_t rows = DIM - (i == I-1 ? pad_I : 0);	
      int start = *(S_indptr + position);
      int end = *(S_indptr + position + rows);
      if(start != end){ //have to check local
	for(size_t j = start; j < end; j++){
	   ...
	   if(index  >= tile_J_start && index < tile_J_start + J*DIM){
	      ...
	      A_mvin[i+1] = A_mvin[i] + DIM;   //mvin DIM rows
	      break; //if detect nonzero data, mvin & break loop
	   }
	   if(j == end - 1){//no need to mvin locally
	     A_mvin[i+1] = A_mvin[i];
	   }
	}
      }else{
	A_mvin[i+1] = A_mvin[i]; //nothing in DIM rows globally
      }
//   printf("%d ", A_mvin[i]);
   }
...
//	printf("\n matmul_table[%d][%d] \n", i, j);
	   ...
	   j_save = j + 1;
	   //overlap mvin A and matmul
	   for (size_t k = 0; k < K; k+=A_blocks) {	//assume A_blocks = 8
	     const size_t blocks = k + A_blocks <= K ? A_blocks : K-k;
//	 printf("\n k: %d, K: %d, A_blocks: %d \n", k, K, A_blocks);
	     const uint32_t A_sp_mvin = A_sp_addr + k*DIM;
	     const size_t cols = DIM * blocks - (k +blocks >= K ? pad_K : 0);
	     if(k%(2*A_blocks)==0){
	        const size_t cols1 = DIM * blocks - (k + 2*blocks >= K ? pad_K : 0);
	        gemmini_extended_mvin(A_dram_addr+k*DIM, A_sp_mvin, cols, rows);   
		gemmini_extended_mvin(A_dram_addr+(k+A_blocks)*DIM, A_sp_mvin+(A_blocks)*DIM, cols1, rows);
	     }
	     const uint32_t B_sp_addr = B_sp_addr_start + k*J*DIM;

  	     gemmini_extended_preload(B_sp_addr+B_mvin[j], C_sp_addr + B_mvin[j], C_cols, DIM, C_cols, rows); //Todo: B_cols, C_cols
     	     gemmini_extended_compute_preloaded(A_sp_mvin, GARBAGE_ADDR, DIM, rows, DIM, DIM);		
  	     gemmini_extended_preload(B_sp_addr+B_mvin[j]+J*DIM, C_sp_addr + B_mvin[j], C_cols, DIM, C_cols, rows); //Todo: B_cols, C_cols
     	     gemmini_extended_compute_preloaded(A_sp_mvin+DIM, GARBAGE_ADDR, DIM, rows, DIM, DIM);		
  	     gemmini_extended_preload(B_sp_addr+B_mvin[j]+2*J*DIM, C_sp_addr + B_mvin[j], C_cols, DIM, C_cols, rows); //Todo: B_cols, C_cols
     	     gemmini_extended_compute_preloaded(A_sp_mvin+2*DIM, GARBAGE_ADDR, DIM, rows, DIM, DIM);		
  	     gemmini_extended_preload(B_sp_addr+B_mvin[j]+3*J*DIM, C_sp_addr + B_mvin[j], C_cols, DIM, C_cols, rows); //Todo: B_cols, C_cols
     	     gemmini_extended_compute_preloaded(A_sp_mvin+3*DIM, GARBAGE_ADDR, DIM, rows, DIM, DIM);		

	     gemmini_extended_preload(B_sp_addr+B_mvin[j]+4*J*DIM, C_sp_addr + B_mvin[j], C_cols, DIM, C_cols, rows); //Todo: B_cols, C_cols
     	     gemmini_extended_compute_preloaded(A_sp_mvin+4*DIM, GARBAGE_ADDR, DIM, rows, DIM, DIM);		
  	     gemmini_extended_preload(B_sp_addr+B_mvin[j]+5*J*DIM, C_sp_addr + B_mvin[j], C_cols, DIM, C_cols, rows); //Todo: B_cols, C_cols
     	     gemmini_extended_compute_preloaded(A_sp_mvin+5*DIM, GARBAGE_ADDR, DIM, rows, DIM, DIM);		
  	     gemmini_extended_preload(B_sp_addr+B_mvin[j]+6*J*DIM, C_sp_addr + B_mvin[j], C_cols, DIM, C_cols, rows); //Todo: B_cols, C_cols
     	     gemmini_extended_compute_preloaded(A_sp_mvin+6*DIM, GARBAGE_ADDR, DIM, rows, DIM, DIM);		
  	     gemmini_extended_preload(B_sp_addr+B_mvin[j]+7*J*DIM, C_sp_addr + B_mvin[j], C_cols, cols-7*DIM, C_cols, rows); //Todo: B_cols, C_cols
     	     gemmini_extended_compute_preloaded(A_sp_mvin+7*DIM, GARBAGE_ADDR, cols-7*DIM, rows, DIM, DIM);		
	
	     for(size_t jj = j_save; jj < J; jj++){
		if(matmul_table[i][jj] == 1){
		   const uint32_t B_sp_addr_inner = B_mvin[jj] + B_sp_addr;
		   const uint32_t C_sp_addr_inner = C_sp_addr + B_mvin[jj];
		   gemmini_extended_preload(B_sp_addr_inner, C_sp_addr_inner, C_cols, DIM, C_cols, rows); //Todo: B_cols, C_cols
     	     	   gemmini_extended_compute_preloaded(A_sp_mvin, GARBAGE_ADDR, DIM, rows, DIM, DIM);		
	  	   gemmini_extended_preload(B_sp_addr_inner+J*DIM, C_sp_addr_inner, C_cols, DIM, C_cols, rows); //Todo: B_cols, C_cols
     		   gemmini_extended_compute_preloaded(A_sp_mvin+DIM, GARBAGE_ADDR, DIM, rows, DIM, DIM);		
  	     	   gemmini_extended_preload(B_sp_addr_inner+2*J*DIM, C_sp_addr_inner, C_cols, DIM, C_cols, rows); //Todo: B_cols, C_cols
     	    	   gemmini_extended_compute_preloaded(A_sp_mvin+2*DIM, GARBAGE_ADDR, DIM, rows, DIM, DIM);		
  	    	   gemmini_extended_preload(B_sp_addr_inner+3*J*DIM, C_sp_addr_inner, C_cols, DIM, C_cols, rows); //Todo: B_cols, C_cols
     	     	   gemmini_extended_compute_preloaded(A_sp_mvin+3*DIM, GARBAGE_ADDR, DIM, rows, DIM, DIM);		

		   gemmini_extended_preload(B_sp_addr_inner+4*J*DIM, C_sp_addr_inner, C_cols, DIM, C_cols, rows); //Todo: B_cols, C_cols
     	     	   gemmini_extended_compute_preloaded(A_sp_mvin+4*DIM, GARBAGE_ADDR, DIM, rows, DIM, DIM);		
	  	   gemmini_extended_preload(B_sp_addr_inner+5*J*DIM, C_sp_addr_inner, C_cols, DIM, C_cols, rows); //Todo: B_cols, C_cols
     		   gemmini_extended_compute_preloaded(A_sp_mvin+5*DIM, GARBAGE_ADDR, DIM, rows, DIM, DIM);		
  	     	   gemmini_extended_preload(B_sp_addr_inner+6*J*DIM, C_sp_addr_inner, C_cols, DIM, C_cols, rows); //Todo: B_cols, C_cols
     	    	   gemmini_extended_compute_preloaded(A_sp_mvin+6*DIM, GARBAGE_ADDR, DIM, rows, DIM, DIM);		
  	    	   gemmini_extended_preload(B_sp_addr_inner+7*J*DIM, C_sp_addr_inner, C_cols, cols-7*DIM, C_cols, rows); //Todo: B_cols, C_cols
     	     	   gemmini_extended_compute_preloaded(A_sp_mvin+7*DIM, GARBAGE_ADDR, cols-7*DIM, rows, DIM, DIM);		
...
/*
  // Compute
  size_t dim_J = J*DIM - pad_J;
  //printf("I: %d, K:%d, J: %d \n", I, K, J);
  for(size_t i = tile_I_start; i < dim_I + tile_I_start; i++){ //considered starting point
    int start = *(S_indptr + i);
    int end = *(S_indptr + (i+1));
    int A_mvin_index = (i - tile_I_start)/DIM;
//    printf("start %d, end %d \n", start, end);
    if(A_mvin[A_mvin_index] != A_mvin[A_mvin_index + 1]){ //if row has been moved in
      for(size_t ii = start; ii < end; ){
  	int index = *(S_index+ii);
//	printf("index: %d, iter: %d\n", index, ii);
  	if(index >= tile_J_start && index < dim_J + tile_J_start){
	  int dim_index = (int)((index-tile_J_start)/DIM);
//	  int j_index = (int)(index/DIM);
//	  printf("row: %d, dim_index: %d \n", i, dim_index);
	  if(B_mvin[dim_index] < B_mvin[dim_index + 1]){ //if column has moved in
	    int B_col[DIM] = {0};
            B_col[(index%DIM)] = 1;
//	    printf("row: %d, dim_index:%d, col: %d \n", i, dim_index, index%DIM);
	    if((*(S_index+ii+1))/DIM == dim_index && ii + 1 < end){
//	       printf("extra: %d, index: %d \n", (*(S_index+ii+1))/DIM, (*(S_index+ii+1))%DIM);
	       B_col[(*(S_index+ii+1))%DIM] = 1;
...
//	    printf("b_col0: %d, b_col1: %d, b_col2: %d, b_col3: %d \n", B_col[0], B_col[1], B_col[2], B_col[3]);
	    //dim_index = dim_index - tile_J_start/DIM;
	    int i_row = (A_mvin[A_mvin_index]);
	    int j_col = B_mvin[dim_index];
//	printf("i_row: %d, j_col: %d \n", i_row, j_col);
            const uint32_t C_sp_addr = C_sp_addr_start + i_row*J + i%DIM + j_col;//dim_index*DIM; 
            const uint32_t A_sp_addr = A_sp_addr_start + i_row*K + i%DIM;
            const uint32_t B_sp_addr = B_sp_addr_start + j_col;//(dim_index)*DIM;//instead of j 
	    for(size_t k = 0; k < K; k++){
               const size_t A_cols = DIM - (k == K - 1 ? pad_K : 0);
//        const size_t A_rows = 1;//DIM - (i == I - 1 ? pad_I : 0);
  	       const size_t B_rows = A_cols;//DIM - (k == K - 1 ? pad_K : 0);
//           const size_t C_cols = DIM - (dim_index == J - 1 ? pad_J : 0); //Todo: how about C_cols?
//        const size_t C_rows = A_rows;
	       gemmini_sddmm_preload(B_sp_addr+k*J*DIM, C_sp_addr, B_col[0], B_col[1], B_col[2], B_col[3], B_rows, B_col[0], B_col[1], B_col[2], B_col[3], 1);
     	       gemmini_extended_compute_preloaded(A_sp_addr+k*DIM, GARBAGE_ADDR, A_cols, 1, DIM, DIM);
	    }
	  }else ii++;
	}
	else ii++;
   ...
   if (C != NULL) {
    for (size_t i = 0; i < I; i++) {
      if(A_mvin[i] < A_mvin[i+1]){ 
       elem_t * const C_dram_addr = C + i*DIM*C_row_stride;
       const uint32_t C_sp_addr = C_sp_addr_start + A_mvin[i]*J;
       const size_t C_rows = DIM - (i == I - 1 ? pad_I : 0);
       int b_point[DIM] = {0};
       for (size_t j = 0; j < J; j++) {
...
static void tiled_matmul_outer_sddmm(size_t dim_I, size_t dim_J, size_t dim_K,
        const elem_t* A, const elem_t* B,
        const acc_t * D, elem_t* C, const int* S_indptr, const int* S_index, const int* S_indtpr_j, const int* S_index_j,
...
//  printf("tile_I: %d, tile_J: %d, tile_K: %d \n", tile_I, tile_J, tile_K);
  ...
  for (size_t i0 = 0; i0 < I0; i0++){
	...
	//tile_I or I?
    const int position = i0*DIM*tile_I; //row position of current tiling factor
...
//  	     const int tile_I_start = i0*tile_I*DIM;
	     const int tile_J_start = j0*tile_J*DIM;

             sp_tiled_matmul_ws_sddmm(A + i0*tile_I*DIM*stride_A + k0*tile_K*DIM,
              B + k0*tile_K*DIM*stride_B + j0*tile_J*DIM,
              pre, out, S_indptr, S_index, S_indtpr_j, S_index_j,
              A_scale_factor, B_scale_factor, D_scale_factor,
              I, J, K,
              pad_I, pad_J, pad_K,
              stride_A, stride_B, stride_D, stride_C,
	      position, tile_J_start, //added for tiling starting point
              no_bias, repeating_bias);
	   }
	} //else printf("zero cols %d \n", j0);
      }
    }//else printf("skip whole rows \n");
  }
  gemmini_fence();
}
...
//for sddmm
void tiled_matmul_sddmm(size_t dim_I, size_t dim_J, size_t dim_K,
        const elem_t* A, const elem_t* B,
        const acc_t * D, elem_t* C, const int* S_indptr, const int* S_index, const int* S_indtpr_j, const int* S_index_j,
        size_t stride_A, size_t stride_B, size_t stride_D, size_t stride_C,
        scale_t A_scale_factor, scale_t B_scale_factor, scale_acc_t D_scale_factor,
  ...
  // Run a tiled matrix multiplication on either Gemmini or the CPU
  if (tiled_matmul_type == OS || tiled_matmul_type == WS) {
      tiled_matmul_outer_sddmm(dim_I, dim_J, dim_K,
              A, B, D, C, S_indptr, S_index, S_indtpr_j, S_index_j,
              stride_A, stride_B, stride_D, stride_C,
              A_scale_factor, B_scale_factor, D_scale_factor,
              tile_I, tile_J, tile_K,
              act, shift, relu6_shift, repeating_bias,
...
void tiled_matmul_auto_sddmm(size_t dim_I, size_t dim_J, size_t dim_K,
        const elem_t* A, const elem_t* B,
        const acc_t * D, elem_t* C, int* S_indptr, int* S_index, int* S_indptr_j, int* S_index_j,
        size_t stride_A, size_t stride_B, size_t stride_D, size_t stride_C,
        scale_t A_scale_factor, scale_t B_scale_factor, scale_acc_t D_scale_factor,
        int act, size_t shift, size_t relu6_shift, bool repeating_bias,
        enum tiled_matmul_type_t tiled_matmul_type) {
...
#define max_tile_k (mats_in_partition / max_tile_i_j)
    ...
    tiled_matmul_sddmm(dim_I, dim_J, dim_K,
        A, B, D, C, S_indptr, S_index, S_indptr_j, S_index_j,
        stride_A, stride_B, stride_D, stride_C,
        A_scale_factor, B_scale_factor, D_scale_factor,
        act, shift, relu6_shift, repeating_bias,
        tile_I, tile_J, tile_K,
