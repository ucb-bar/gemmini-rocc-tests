...
void sp_tiled_conv_dw(
    ...
    const int irows = orows * stride + kernel_dim - 1; // - 2 * padding;
    const int icols = ocols * stride + kernel_dim - 1; // - 2 * padding;
...
//        const int J = ochs - och > DIM ? DIM : ochs - och;
		...
		int kch = 0; int och = 0;
                    ...
                    gemmini_extended_mvin(weights + (krow*kernel_dim + kcol + kch) * out_channels + och,
                        B_sp_addr,
                        1, 1);
        }
  }
...
//   printf("compute \n");
   for (int b = 0; b < batches; b++)
        for (int orow = 0; orow < orows; orow++)
            for (int ocol = 0; ocol < ocols; ocol += DIM) {
                const int I = ocols - ocol > DIM ? DIM : ocols - ocol;
		int och = 0;int kch = 0;int J = 1; int K = 1;
//                for (int och = 0; och < ochs; och += DIM) {
//                    const int J = ochs - och > DIM ? DIM : ochs - och;
...
//                                const int K = kchs - kch > DIM ? DIM : kchs - kch;
                                ...
                                //    C_sp_addr;
				//printf("C_sp_addr: %d, irow: %d, icol: %d \n", C_sp_addr, irow, icol);
