//instantiate tiling at dense area
   ...
   for (size_t i = 0; i < I; i++) {
	...
	for(size_t j = start; j < end; j++){
	   ...
	   if(index  >= tile_J_start && index < tile_J_start + J*DIM){
	      const int dim_index = (int)((index - tile_J_start)/DIM);
	      ...
	      A_mvin[i+1] += DIM;   //mvin DIM rows
	      break; //if detect nonzero data, mvin & break loop
	   }
	}
	if(A_mvin[i+1] != A_mvin[i]) //have something
	   for(size_t j = 0; j < dim_J; ){
		int col_pos = j + tile_J_start;
		int col_start = *(S_indptr_j + col_pos);
		int col_end = *(S_indptr_j + col_pos + 1);
		if(*(S_index_j+col_start) < tile_I_start+I*DIM && *(S_index_j+col_end-1)>=tile_I_start){
		    for(size_t jj = col_start; jj < col_end; jj++){
			int row = (int)((jj-tile_I_start)/DIM);
			if(row == i){
			   int dim_index = (int)(j/DIM);
			   matmul_table[i][dim_index] = 1;
			   j = (dim_index + 1)*DIM;
			   break;
			}
		    }
  ...
  printf("B mvin : ");
	...
	for(size_t i = 0; i < I; i++){
	   if(matmul_table[i][j] != 0){
		...
		if(matmul_table[ii][j]!=0){
		   //printf("matmul_table[%d][%d] \n", ii, j);
		   const uint32_t A_sp_addr = A_sp_addr_start + A_mvin[ii]*K;//sp address based on mvin rows
	           const uint32_t C_sp_addr = C_sp_addr_start + B_mvin[j] + A_mvin[ii]*J;
		   const size_t C_rows = DIM - (ii = I - 1 ? pad_I : 0);
		   gemmini_extended_preload(B_sp_addr, C_sp_addr, cols, rows0, cols, C_rows); //Todo: B_cols, C_cols
     		   gemmini_extended_compute_preloaded(A_sp_addr, GARBAGE_ADDR, rows0, C_rows, DIM, DIM);		
		}
	      }
	      B_mvin[j+1] = B_mvin[j] + DIM;
	      B_mvin_col[B_mvin_num] = j;
	      B_mvin_num++;
  ...
  printf(" \n");
   ...
   for (size_t k = 1; k < K; k++) {
     ...
     for (size_t j = 0; j < B_mvin_num; j += 1) {
	  ...
	  const int real_col = B_mvin_col[j];
	  gemmini_extended_mvin(B_dram_addr+DIM*real_col, B_sp_addr+j*DIM, cols, rows);
	  const uint32_t C_sp_addr = C_sp_addr_start + real_col*DIM;
	  for(size_t i = 0; i < I; i++){
	    if(matmul_table[i][real_col]==1){
		const uint32_t A_sp_addr = A_sp_addr_start + A_mvin[i]*K;//sp address based on mvin rows 
		const size_t C_rows = DIM - (i==I-1? pad_I:0);
	     	gemmini_extended_preload(B_sp_addr+j*DIM, C_sp_addr+A_mvin[i]*J, cols, rows, cols, C_rows); //Todo: B_cols, C_cols
     	     	gemmini_extended_compute_preloaded(A_sp_addr+k*DIM, GARBAGE_ADDR, rows, C_rows, DIM, DIM);		
