...
// weight-stationary matmul loop
 #define gemmini_loop_ws(B, a_mvin, b_mvin, I, J, K, rows, cols, pad_K, transpose) \
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC, ((uint64_t)(B) << 32) | ((uint64_t)(a_mvin) << 16) | (uint64_t)(b_mvin), ((uint64_t)(pad_K) << 41) | ((uint64_t)(cols) << 36) | ((uint64_t)(rows) << 31) | ((uint64_t)(transpose) << 30) | ((uint64_t)(K) << 20) | ((uint64_t)(J) << 10) | ((uint64_t)(I)), k_LOOP_WS)
...
static void sp_tiled_matmul_ws_sddmm(const elem_t * A, const elem_t * Bt,
...
//start = read_cyclesh();
  for (size_t i = 0; i < J; i++) {
      int position = i*DIM + tile_J_start;//current tile location for whole matrix
      const size_t cols = DIM - (i == J-1 ? pad_J : 0);	
      int start = *(S_indptr_j + position);
      int end = *(S_indptr_j + position + cols);
      if(start != end){ //have to check local
	for(size_t j = start; j < end; j++){
	   int index = *(S_index_j + j);
	   if(index  >= tile_I_start && index < tile_I_start + I*DIM){
	      const elem_t * const B_dram_addr = Bt + i*B_row_stride*DIM;
	      const uint32_t B_sp_addr = B_sp_addr_start + B_mvin[i]*K;//sp address based on mvin rows
	      for (size_t k = 0; k < K; k += B_blocks) {	
	        const size_t blocks = k + B_blocks <= K ? B_blocks : K-k;
	        const size_t rows = blocks * DIM - (k + blocks >= K ? pad_K : 0);
	        gemmini_extended_mvin(B_dram_addr+k*DIM, B_sp_addr+k*DIM, rows, cols);   
	      }
	      B_mvin[i+1] = B_mvin[i] + DIM;   //mvin DIM rows
	...
	B_mvin[i+1] = B_mvin[i]; //nothing in DIM rows globally
      }
//   printf("%d ", B_mvin[i]);
   }
//  printf("%d \n", B_mvin[J]);
...
*/
   ...
   for(size_t i = 0; i < I; i++)
	...
	const uint32_t A_sp_addr = A_sp_addr_start + A_mvin[i]*K;//sp address based on mvin rows
	const size_t rows = DIM - (i==I-1 ? pad_I : 0);
	int point = tile_I_start + i*DIM;
	int start = *(S_indptr + point);
	int end = *(S_indptr + point + rows);
	for(size_t j = 0; j < J; j++){
	   if(B_mvin[j] != B_mvin[j+1]){
	      for(size_t jj = start; jj < end; jj++){
		 int index = *(S_index + jj);
		 if(index >= tile_J_start + j*DIM && index < tile_J_start + (j+1)*DIM){ 
//           	    const uint32_t C_sp_addr = C_sp_addr_start + A_mvin[i]*J + B_mvin[j];
//	   	    const uint32_t B_sp_addr = B_sp_addr_start + B_mvin[j]*K;		
	   	    const size_t cols = DIM - ((j == J - 1) ? pad_J : 0);
/*
 		    for(size_t k = 0; k < K; k++){
		     	gemmini_extended_preload(B_sp_addr+k*DIM, C_sp_addr, DIM, cols, cols, rows); //Todo: B_cols, C_cols
...
static void tiled_matmul_outer_sddmm(size_t dim_I, size_t dim_J, size_t dim_K,
        const elem_t* A, const elem_t* Bt,
        const acc_t * D, elem_t* C, const int* S_indptr, const int* S_index, const int* S_indptr_j, const int* S_index_j,
...
//  printf("tile_I: %d, tile_J: %d, tile_K: %d \n", tile_I, tile_J, tile_K);
  ...
  gemmini_extended_config_ex(dataflow, act, 0, shift, relu6_shift, 1, A_transpose, B_transpose)
...
//	     const int tile_J_start = j0*tile_J*DIM;
             ...
             sp_tiled_matmul_ws_sddmm(A + i0*tile_I*DIM*stride_A + k0*tile_K*DIM,
              ...
              Bt + j0*tile_J*DIM*stride_B + k0*tile_K*DIM,
	      pre, out, S_indptr, S_index, S_indptr_j, S_index_j,
...
static void sddmm_cpu(size_t DIM_I, size_t DIM_J, size_t DIM_K,
        const elem_t* A, const elem_t* B, const acc_t * D,
        elem_t* C, const int* S_indptr, const int* S_index,
        size_t stride_A, size_t stride_B, size_t stride_D, size_t stride_C,
        scale_t A_scale_factor, scale_t B_scale_factor, scale_acc_t D_scale_factor,
        int act, size_t shift, size_t relu6_shift, bool repeating_bias) {
...
//	printf("i : %d, j: %d, jj: %d \n", i, j, jj);
        ...
        acc_t result = no_bias ? 0 : GEMMINI_SCALE(*(D + bias_row * stride_D + jj), D_scale_factor);

        for (size_t k = 0; k < DIM_K; k++) {
          acc_t past_opixel = result;
          result += GEMMINI_SCALE(*(A + i*stride_A + k), A_scale_factor) * GEMMINI_SCALE(*((elem_t*)B + k*stride_B + jj), B_scale_factor);
        }

        *(C + i*stride_C + jj) = scale_and_sat(result, act, shift, relu6_shift);
      }
    }
  }
}
...
void tiled_matmul_sddmm(size_t dim_I, size_t dim_J, size_t dim_K,
  ...
  if (tiled_matmul_type == OS || tiled_matmul_type == WS) {
      ...
      sddmm_cpu(dim_I, dim_J, dim_K,
              A, B, D, C, S_indptr, S_index,
              stride_A, stride_B, stride_D, stride_C,
              A_scale_factor, B_scale_factor, D_scale_factor,
              act, shift, relu6_shift, repeating_bias);
  }
