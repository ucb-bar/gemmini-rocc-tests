...
static void sp_tiled_conv_tutorial(
  ...
  // mvin input
  {
    ...
    gemmini_extended4_config_ld(dram_stride, MVIN_SCALE_IDENTITY, false, spad_stride, 0);
    ...
    for (int b = 0; b < batches; b++)
      for (int irow = 0; irow < irows; irow++) {
        ...
        for (int icol = 0; icol < icols; icol += DIM) {
          ...
          for (int ich = 0; ich < ichs; ich += max_chs_per_mvin) {
            int K = ichs - ich > max_chs_per_mvin ?
              max_chs_per_mvin : ichs - ich;

            uint32_t A_sp_addr = A_sp_addr_start + (ich / DIM) * batches * irows * icols + b * irows * icols + irow * icols + icol;
  ...
  // mvin weights
  {
    const int max_chs_per_mvin = ochs < MAX_BLOCK_LEN * DIM ? ochs :
        MAX_BLOCK_LEN * DIM;

    const size_t dram_stride = out_channels * sizeof(elem_t);
    ...
    for (int och = 0; och < ochs; och += max_chs_per_mvin) {
      for (int krow = 0; krow < krows; krow++)
        for (int kcol = 0; kcol < kcols; kcol++)
          for (int kch = 0; kch < kchs; kch += DIM) {
            ...
            const int J = ochs - och > max_chs_per_mvin ? max_chs_per_mvin : ochs - och;
...
static void tiled_conv_tutorial(
        int batch_size, int in_dim, int in_channels,
        int out_channels, int out_dim,
        int stride, int kernel_dim,
    ...
    const size_t st_dram_stride = out_channels * sizeof(elem_t);
    gemmini_extended_config_st(st_dram_stride, act, scale);

    gemmini_extended3_config_ex(WEIGHT_STATIONARY, 0, 0, 0, 0, 1, stride, false, false, 0, 0, 0, 0, 0, 0, 0, 0, 0, false);
...
static void tiled_conv_tutorial_auto(
        int batch_size, int in_dim, int in_channels,
        int out_channels, int out_dim,
        int stride, int kernel_dim,
    ...
    const int max_args[] = {batch_size, out_dim, out_dim, out_channels, kernel_dim, kernel_dim, in_channels};
    ...
    int spad_rows = tiled_conv_total_spad_rows_A_stride(false,
        ...
        args[0], args[1], args[2], args[3], args[4], args[5], args[6], 1, 1);
    ...
    while (!not_increased) {
        ...
        spad_rows = tiled_conv_total_spad_rows_A_stride(false,
            stride, 1, 1, false, false, false,
            args_candidate[0], args_candidate[1], args_candidate[2], args_candidate[3], args_candidate[4], args_candidate[5], args_candidate[6], 1, 1);
        acc_rows = tiled_conv_total_spad_rows_A_stride(true,
            stride, 1, 1, false, false, false,
            args_candidate[0], args_candidate[1], args_candidate[2], args_candidate[3], args_candidate[4], args_candidate[5], args_candidate[6], 1, 1);
