...
#define RAND rand()
    ...
    uint32_t static_alloc_start, activation_double_buffer;
    ...
    activation_double_buffer = 00000;
    ...
    unsigned long mvin_cycles, mvout_cycles, layer0_cycles, layer1_cycles, layer2_cycles;
    unsigned long cpu_layer0_cycles, cpu_layer1_cycles, cpu_layer2_cycles;


    printf("==================================\n");
    printf("EE290-2 Lab2 Full Scratchpad\n");
    printf("==================================\n");
    printf("Executing Gemmini MLP\n");
    printf("----------------------------------\n");
    printf("Starting input mvin\n");
    start = read_cycles();

    // TODO LAB2
    mvin_matrix(BATCH_SIZE, INPUT_SIZE, input_mat, 00000);

    end = read_cycles();
    mvin_cycles = end-start;
    ...
    // TODO LAB2
    sp_tiled_matmul_auto_full_spad_ws(BATCH_SIZE, HIDDEN_SIZE, INPUT_SIZE,
             00000, weights0_sp, NULL, 00000, RELU);
    ...
    mvout_matrix(BATCH_SIZE, HIDDEN_SIZE, 00000, inter_results0);
    ...
    // TODO LAB2
    sp_tiled_matmul_auto_full_spad_ws(BATCH_SIZE, HIDDEN_SIZE, HIDDEN_SIZE,
            00000, weights1_sp, NULL, 00000, RELU);
    ...
    // TODO LAB2
    // Used for functional testing
    mvout_matrix(BATCH_SIZE, HIDDEN_SIZE, 00000, inter_results1);

    printf("Starting gemmini matmul\n");
    start = read_cycles();

    // TODO LAB2
    sp_tiled_matmul_auto_full_spad_ws(BATCH_SIZE, OUTPUT_SIZE, HIDDEN_SIZE,
            00000, weights2_sp, NULL, 00000, RELU);

    end = read_cycles();
    layer2_cycles = end-start;
    printf("Cycles taken: %u\n", layer2_cycles);

    printf("Starting output mvout\n");
    start = read_cycles();

    // TODO LAB2
    mvout_matrix(BATCH_SIZE, OUTPUT_SIZE, 00000, inter_results2);

    end = read_cycles();
    mvout_cycles = end-start;
    printf("Cycles taken: %u\n", mvout_cycles);
