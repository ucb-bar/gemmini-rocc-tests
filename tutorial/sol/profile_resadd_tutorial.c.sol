...
int main (int argc, char * argv[]) {
    ...
    /*
     * TUTORIAL
     * Add code here to configure "total_bytes_read" and
     * "total_stall_cycles_for_l2_access" counters.
     *
     ...
     */
    uint32_t total_bytes_read, total_stall_cycles_for_l2_access;
    counter_reset();

    counter_configure(0, RDMA_BYTES_REC);
    counter_configure(1, RDMA_TLB_WAIT_CYCLES);
    ...
    total_stall_cycles_for_l2_access = counter_read(1);

    printf("Total cycles taken for resadd: %llu\n", total_cycles);
    printf("Read DMA bandwidth (bytes/cycle): %llu\n", total_bytes_read / total_cycles);
    printf("Read DMA total stall cycles for L2 access: %llu\n", total_stall_cycles_for_l2_access);
