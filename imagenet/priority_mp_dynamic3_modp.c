...
//  pthread_barrier_wait(&barrier[NUM_CORE]);
  ...
  for(int g = 0; g < queue_group; g++){
	...
	if(!others_done || status > 0){
	    ...
	    if(cid == 0) {
		gemmini_score[group_id] = MAX(1, (int)(10 * (temp_cycles - total_queue_dispatch[queue_id]))/total_queue_target[queue_id]);
	    printf("gemmini_score: %d\n", gemmini_score[group_id]);
...
int main (int argc, char * argv[]) {
   ...
   //     nn_args[i].target_cycles = RESNET_TARGET;
    ...
    workload_mode_2(total_workloads, BATCH1, BATCH2, BATCH4, SEED, CAP, TARGET_SCALE, CAP_SCALE); 
