...
//#include "kwsnet50_mt_images.h"
#define NUM_OUTPUT (25+1+11+4)

#define num_proc 8
#define num_layer 30
#define num_resadd 11

pthread_barrier_t barrier[num_proc];

#include "funct_kwsnet_4.h"

#define BATCH_DIVIDE 4
...
struct thread_args{
    uint64_t total_thread_cycles, total_cycles, total_conv_cycles, total_matmul_cycles, total_resadd_cycles, total_pool_cycles;
...
void *thread_NN(void *arg){
    ...
    cycles = kwsnet_function_4(cid, 0, true, true, OROW_DIVIDE, BATCH_DIVIDE, target_util, &barrier[nn_args->barrier_index]);

    uint64_t thread_end = read_cycles();
    nn_args->total_thread_cycles = thread_end - thread_start;
    nn_args->total_conv_cycles = *(cycles+37);
    nn_args->total_pool_cycles = *(cycles+38);
    //nn_args->other_cycles = other_cycles;
    nn_args->total_resadd_cycles = *(cycles+39);
    nn_args->total_cycles = *(cycles+40);
    for(int i = 0; i < NUM_OUTPUT; i++){
	if(i < 25) nn_args->conv_cycles[i] = *(cycles+i);
	else if(i < 26) nn_args->pool_cycles[i-25] = *(cycles+i);
	else if(i < 37) nn_args->resadd_cycles[i-26] = *(cycles+i);
    }
...
//	 printf("kwsnet repeat %d total cycles with threading overhead: %llu \n", r, end - start);

	 uint64_t thread_kwsnet_max = 0;
	 uint64_t total_kwsnet_max = 0;
	 for(int i = 0; i < num_proc; i++){
		  ...
		  thread_kwsnet_max = thread_kwsnet_max > thread_cycles ? thread_kwsnet_max : thread_cycles;
		  total_kwsnet_max = total_kwsnet_max > total_cycles ? total_kwsnet_max : total_cycles;
	 }
	  printf("\nkwsnet repeat %d total thread cycles: %llu\n", r, thread_kwsnet_max);
	  printf("kwsnet repeat %d total cycles: %llu\n", r, total_kwsnet_max);
...

	 {
		  ...
		  printf("kwsnet repeat %d Conv layer %d worst cycles: %llu \n", r, i, max);
		  max = 0;
	 }
...

	 {
		  ...
		  printf("kwsnet repeat %d Pool layer %d worst cycles: %llu \n", r, i, max);
		  max = 0;
...

	 {
		  ...
		  printf("kwsnet repeat %d Resadd layer %d worst cycles: %llu \n", r, i, max);
		  max = 0;
