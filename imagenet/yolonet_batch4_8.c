...
//#include "yolonet50_mt_images.h"
#define NUM_OUTPUT (19+5+3)
...
void *thread_NN(void *arg){
    ...
    cycles = yolonet_function_4(cid, 0, true, true, true, OROW_DIVIDE, BATCH_DIVIDE, target_util, &barrier[nn_args->barrier_index]);

    uint64_t thread_end = read_cycles();
    nn_args->total_thread_cycles = thread_end - thread_start;
    nn_args->total_conv_cycles = *(cycles+24);
    nn_args->total_pool_cycles = *(cycles+25);
    nn_args->total_cycles = *(cycles+26);
    for(int i = 0; i < NUM_OUTPUT; i++){
	if(i < 19) nn_args->conv_cycles[i] = *(cycles+i);
	else if(i < 24) nn_args->pool_cycles[i-19] = *(cycles+i);
    }
...
//	 printf("yolonet repeat %d total cycles with threading overhead: %llu \n", r, end - start);

	 uint64_t thread_yolonet_max = 0;
	 uint64_t total_yolonet_max = 0;
	 for(int i = 0; i < num_proc; i++){
		  ...
		  thread_yolonet_max = thread_yolonet_max > thread_cycles ? thread_yolonet_max : thread_cycles;
		  total_yolonet_max = total_yolonet_max > total_cycles ? total_yolonet_max : total_cycles;
	 }
	  printf("\nyolonet repeat %d total thread cycles: %llu\n", r, thread_yolonet_max);
	  printf("yolonet repeat %d total cycles: %llu\n", r, total_yolonet_max);
...

	 {
		  ...
		  printf("yolonet repeat %d Conv layer %d worst cycles: %llu \n", r, i, max);
		  max = 0;
	 }
...

	 {
		  ...
		  printf("yolonet repeat %d Matmul layer %d worst cycles: %llu \n", r, i, max);
		  max = 0;
...

	 {
		  ...
		  printf("yolonet repeat %d Pool layer %d worst cycles: %llu \n", r, i, max);
		  max = 0;
