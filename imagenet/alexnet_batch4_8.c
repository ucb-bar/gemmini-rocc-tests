...
void *thread_NN(void *arg){
    ...
    cycles = alexnet_function_4(cid, 0, true, true, OROW_DIVIDE, BATCH_DIVIDE, target_util, &barrier[nn_args->barrier_index]);
...
//	 printf("alexnet repeat %d total cycles with threading overhead: %llu \n", r, end - start);
	 ...
	 uint64_t thread_alexnet_max = 0;
	 uint64_t total_alexnet_max = 0;
	 for(int i = 0; i < num_proc; i++){
