...
void *thread_NN(void *arg){
  ...
  for(int g = 0; g < queue_group; g++){
     for(int i = 0; i < QUEUE_DEPTH; i++){
       ...
       if(queue_id != -1){
          ...
          if(status < workload_group[workload_id]){
            ...
            if(group_queue_id >= 0){
              total_runtime = workload_group_function(queue_id, group_queue_id, workload_id, total_queue_type[group_queue_id], cid, workload_num_core);
              end = read_cycles();
              queue_id = (cid == 0) ? queue_id : group_queue_id;
            }
            else{ // no grouped workload to execute
              total_runtime = workload_function(queue_id, workload_id, cid, workload_num_core, &barrier[nn_args->barrier_index]);
              end = read_cycles();
            }
    //printf("rid: %d, workload id: %d, queue id: %d\n", real_cid, workload_id, queue_id);
            ...
            total_queue_finish[real_cid][queue_id] = (temp_cycles + end - inner_start - total_queue_dispatch[queue_id]);
...
int main (int argc, char * argv[]) {
   ...
   //     nn_args[i].target_cycles = RESNET_TARGET;
    ...
    workload_mode_1(QoS, total_workloads, BATCH1, BATCH2, BATCH4, SEED, CAP, TARGET_SCALE, CAP_SCALE); 
