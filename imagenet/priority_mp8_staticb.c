...
void *thread_NN(void *arg){
  ...
  printf("entered thread_NN - cid: %d, total_sub_group_id(barrier_index): %d, sub_group_id: %d, group_id: %d, real_cid: %d, queue_group: %d \n", cid, total_sub_group_id, sub_group_id, group_id, real_cid, queue_group);
  uint64_t start, end;
...
//printf("queue id: %d, workload id: %d, others done: %d\n", queue_id, workload_id, others_done);
       if(queue_id != -1){
         if(!others_done){
	   if(cid == 0) {
             gemmini_score[total_sub_group_id] = total_queue_priority[queue_id];// (1 + total_queue_priority[queue_id]) / 4 + MAX(1, (int)(4 * (temp_cycles - total_queue_dispatch[queue_id]))/total_queue_target[queue_id]);
           }	

           uint64_t inner_start = read_cycles();
        //   uint64_t total_runtime = workload_function(queue_id, workload_id, cid, group_id, total_sub_group_id, all ? SUB_CORE : workload_num_core, 0, &barrier[nn_args->barrier_index]);
           // iterate
           uint64_t total_runtime = workload_function(queue_id, workload_id, all ? group_cid : cid, group_id, total_sub_group_id, all ? SUB_CORE : workload_num_core, 0, all ? &barrier_sub[group_id] : &barrier[total_sub_group_id]);
