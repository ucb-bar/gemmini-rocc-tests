...
//#include "fcnnet50_mt_images.h"
#define NUM_OUTPUT (19+36+16+3)
...
void *thread_NN(void *arg){
    ...
    cycles = fcnnet_function_8(cid, 0, OROW_DIVIDE, BATCH_DIVIDE, target_util, &barrier[nn_args->barrier_index]);

    uint64_t thread_end = read_cycles();
    nn_args->total_thread_cycles = thread_end - thread_start;
    nn_args->total_conv_cycles = *(cycles+71);
    //nn_args->other_cycles = other_cycles;
    nn_args->total_resadd_cycles = *(cycles+72);
    nn_args->total_cycles = *(cycles+73);
    for(int i = 0; i < NUM_OUTPUT; i++){
	if(i < 55) nn_args->conv_cycles[i] = *(cycles+i);
	else if(i < 71) nn_args->resadd_cycles[i-55] = *(cycles+i);
    }
...
//	 printf("fcnnet repeat %d total cycles with threading overhead: %llu \n", r, end - start);

	 uint64_t thread_fcnnet_max = 0;
	 uint64_t total_fcnnet_max = 0;
	 for(int i = 0; i < num_proc; i++){
		  ...
		  thread_fcnnet_max = thread_fcnnet_max > thread_cycles ? thread_fcnnet_max : thread_cycles;
		  total_fcnnet_max = total_fcnnet_max > total_cycles ? total_fcnnet_max : total_cycles;
	 }
	  printf("\nfcnnet repeat %d total thread cycles: %llu\n", r, thread_fcnnet_max);
	  printf("fcnnet repeat %d total cycles: %llu\n", r, total_fcnnet_max);
...

	 {
		  ...
		  printf("fcnnet repeat %d Conv layer %d worst cycles: %llu \n", r, i, max);
		  max = 0;
	 }
...
/*
...

	 {
		  ...
		  printf("fcnnet repeat %d Matmul layer %d worst cycles: %llu \n", r, i, max);
		  max = 0;
...
*/
...

	 {
		  ...
		  printf("fcnnet repeat %d Resadd layer %d worst cycles: %llu \n", r, i, max);
		  max = 0;
