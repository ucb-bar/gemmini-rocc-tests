...
void *thread_NN(void *arg){
  ...
  for(int g = 0; g < queue_group; g++){
	...
	if(!others_done || status > 0){
	    ...
	    if(cid == 0) {
	       gemmini_score[group_id] = (int)(total_queue_priority[queue_id] * 100000 + ((100000*(temp_cycles - total_queue_dispatch[queue_id])) / sp_prediction_cycles[total_queue_qos[queue_id]][workload_id]));
	    }
            int group_queue_id = gemmini_workload_grouped[group_id][g][i];
    	//printf("workload id: %d, queue id: %d, status: %d, group queue id: %d, score: %d\n", workload_id, queue_id, status, group_queue_id, gemmini_score[group_id]);
            if(group_queue_id >= 0){
              total_runtime = workload_group_function(queue_id, group_queue_id, workload_id, total_queue_type[group_queue_id], cid, group_id, workload_num_core, -1, &barrier[nn_args->barrier_index]);
              ...
              total_runtime = workload_function(queue_id, workload_id, cid, group_id, workload_num_core, -1, &barrier[nn_args->barrier_index]);
              end = read_cycles();
            }
            pthread_barrier_wait(&barrier[nn_args->barrier_index]);
