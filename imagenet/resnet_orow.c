...
void *thread_NN(void *arg){
    ...
    *cycles = resnet_function(cid, NUM_OUTPUT, cycles, OROW_DIVIDE, BATCH_DIVIDE, target_util, &barrier[nn_args->barrier_index]);

    uint64_t thread_end = read_cycles();
    printf("finished resnet function - cid: %d\n", cid);
	...
	if(i < 20) nn_args->conv_cycles[i] = cycles[i];
	else if(i < 54) nn_args->matmul_cycles[i-20] = cycles[i];
...
/*
	 for(int i = 0; i < OROW_DIVIDE; i++){
		  ...
		  uint64_t total_cycles =  conv_cycles + matmul_cycles + resadd_cycles + other_cycles;
		  uint64_t thread_cycles = nn_args[i].total_thread_cycles;


		  printf("\nresnet proc %d total thread cycles: %llu\n", i, thread_cycles);
		  printf("resnet proc %d Total cycles: %llu (100%%)\n", i, total_cycles);
		  printf("resnet proc %d Matmul cycles: %llu (%d%%)\n", i, matmul_cycles, (matmul_cycles * 100) / total_cycles);
		  printf("resnet proc %d Conv cycles: %llu (%d%%)\n", i, conv_cycles, (conv_cycles * 100) / total_cycles);
		  //printf("Depthwise convolution cycles: %llu (%d%%)\n", conv_dw_cycles, (conv_dw_cycles * 100) / total_cycles);
		  printf("resnet proc %d Resadd cycles: %llu (%d%%)\n", i, resadd_cycles, (resadd_cycles * 100) / total_cycles);
		  printf("resnet proc %d Other cycles: %llu (%d%%)\n", i, other_cycles, (other_cycles * 100) / total_cycles);
		  printf("==================================\n");
