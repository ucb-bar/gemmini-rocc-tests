...
#define NUM_CORE 8
#define SEED 2
#define total_workloads 235 // 100 each
...
//       if(cid == 0 && workload_num == 1) gemmini_last[sub_group_id] = true;
 ...
 //            total_runtime += workload_planaria_function(queue_id, workload_id, all ? group_cid : cid, group_id, all ? total_sub_group_id / SUB_GROUP : total_sub_group_id, all ? SUB_CORE : workload_num_core, slack_time, all ? &barrier_sub[group_id] : &barrier[total_sub_group_id]);
	    if(cid == 0 && gemmini_terminate_receive[total_sub_group_id] == 1){
		if(gemmini_terminate[total_sub_group_id] && gemmini_terminate[other_total_sub_group_id]) gemmini_terminate_receive[total_sub_group_id] = 3;
		else if(gemmini_terminate[other_total_sub_group_id]) gemmini_terminate_receive[total_sub_group_id] = 2;
		else if(gemmini_terminate[total_sub_group_id]) gemmini_terminate_receive[total_sub_group_id] = 1;
		else gemmini_terminate_receive[total_sub_group_id] = 0;
	    }
            ...
            if(!inner_done && gemmini_terminate_receive[total_sub_group_id] != 0){
              if(gemmini_terminate_receive[total_sub_group_id] == 1){ // other made it terminate
  ...
  //	total_queue_planaria_status[group_id][queue_id] = 1;
              ...
              else if(gemmini_terminate_receive[total_sub_group_id] == 2){
                pthread_barrier_wait(&barrier_finish[total_sub_group_id]);
            ...
            else if(inner_done && gemmini_terminate_receive[total_sub_group_id] != 0){
