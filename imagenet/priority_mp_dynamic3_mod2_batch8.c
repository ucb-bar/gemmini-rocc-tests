...
void *thread_NN(void *arg){
  ...
  for(int g = 0; g < queue_group; g++){
	...
	if(!others_done ){
	    ...
	    if(cid == 0) {
		gemmini_score[group_id] = (1 + total_queue_priority[queue_id]) / 4 + MAX(1, (int)(4 * (temp_cycles - total_queue_dispatch[queue_id]))/total_queue_target[queue_id]);
	   }	 
    	...
    	//printf("workload id: %d, queue id: %d, status: %d, group queue id: %d, score: %d\n", workload_id, queue_id, status, group_queue_id, gemmini_score[group_id]);
              total_runtime = workload_batch_function(queue_id, workload_id, cid, group_id, workload_num_core, 0, &barrier[nn_args->barrier_index]);
