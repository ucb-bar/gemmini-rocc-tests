...
//#include "squeezenet50_mt_images.h"
#define NUM_OUTPUT (11+15+1+3)
...
void *thread_NN(void *arg){
    ...
    cycles = squeezenet_function_8(cid, 0,  OROW_DIVIDE, BATCH_DIVIDE, target_util, &barrier[nn_args->barrier_index]);

    uint64_t thread_end = read_cycles();
    nn_args->total_thread_cycles = thread_end - thread_start;
    nn_args->total_conv_cycles = *(cycles+27);
    //nn_args->other_cycles = other_cycles;
    nn_args->total_pool_cycles = *(cycles+28);
    nn_args->total_cycles = *(cycles+29);
    for(int i = 0; i < NUM_OUTPUT; i++){
	if(i < 26) nn_args->conv_cycles[i] = *(cycles+i);
	else if(i < 27) nn_args->pool_cycles[i-26] = *(cycles+i);
    }
...
//	 printf("squeezenet repeat %d total cycles with threading overhead: %llu \n", r, end - start);
	 ...
	 uint64_t thread_squeezenet_max = 0;
	 uint64_t total_squeezenet_max = 0;
	 for(int i = 0; i < num_proc; i++){
		  ...
		  thread_squeezenet_max = thread_squeezenet_max > thread_cycles ? thread_squeezenet_max : thread_cycles;
		  total_squeezenet_max = total_squeezenet_max > total_cycles ? total_squeezenet_max : total_cycles;
	 }
