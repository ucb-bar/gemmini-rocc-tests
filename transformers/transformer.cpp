        ...
        elem_t * attn_buf, elem_t * out_buf, elem_t * resadd_buf)
{
    ...
    for (int i = 0; i < qkv_matmuls_n; i++) {
        ...
        const elem_t * qkv_outs[] = {Q_buf, K_buf, V_buf};

        const elem_t * qkv_w = qkv_weights[i];
        const elem_t * qkv_out = qkv_outs[i];
    ...
    for (int head = 0; head < num_heads; head++) {
        ...
        const elem_t * C = attn_buf + head * seq_len * seq_len;
    ...
    for (int head = 0; head < num_heads; head++) {
        ...
        const elem_t * C = out_buf + head * hidden_dim_per_head;
